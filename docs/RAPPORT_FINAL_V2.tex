...
\item \textbf{Chambre des représentants — \emph{House Clerk}.} Source publique sans barrière~: une archive
...
\item \textbf{Sénat — \emph{eFD}} (\emph{Electronic Financial Disclosure}). L'API de recherche du Sénat
...
\item \textbf{Référentiel des élus — \emph{congress-legislators}.}\label{sec:reference} Une déclaration ne
donne qu'un \emph{nom}~; ce jeu de données ouvert fournit, pour chaque membre, son identifiant officiel
(\co{bioguide_id}), son \textbf{parti} et ses \textbf{commissions} — avec un repli sur des YAML embarqués
dans le dépôt pour rester reproductible hors-ligne.
\item \textbf{Lecture des scans — \emph{Claude Vision}.}\label{sec:vision} La moitié des déclarations de la
...
\item \textbf{Validation — \emph{Quiver Quantitative}.} Agrégateur commercial des transactions du Congrès,
...
(\co{build_report}) y répond par \textbf{six contrôles} et quatre figures, calculés en \textbf{lecture
seule} des tables finales, sans aucun appel d'API (matplotlib en mode \co{Agg})~:
