...
validation, qualité, orchestration~; ce qui dépend de la source — matcher d'identité, montants, ticker,
OCR de production, réconciliation Quiver — vit dans \co{house/} et \co{senate/}. Les données et les tests
...
\co{quiver}\dots)~; seules leurs implémentations diffèrent, là où les sources diffèrent. L'unique
asymétrie de structure est \co{senate/fusion.py} (§\ref{sec:socr})~: côté Chambre des représentants, la
fusion électronique+OCR vit dans \co{house/ocr.py}.

\subsection{Le flux de bout en bout (le schéma à lire en premier)}
\label{sec:pipeline}
Le schéma ci-dessous se lit \textbf{en premier}~: il montre, de bout en bout, comment une déclaration
devient une ligne de la table finale. En haut, les \textbf{deux sources} (Chambre des représentants
\emph{vs} Sénat)~; au milieu, la \textbf{fourche électronique / scanné} qui oriente chaque document vers
son extraction~; en bas, le \textbf{tronc commun} partagé par les deux chambres — identité,
enrichissement, table finale, validation. L'orchestration est tenue par \co{common/pipeline.py}
(\co{build_steps})~: \textbf{6 étapes} en sous-processus, suivies d'un \textbf{rapport qualité} hors-ligne,
en lecture seule (\co{common/quality.py}).
...
\node[step=houseC, text width=3.0cm] (hpar) at (-6.0,-4.3) {\textbf{parsing déterministe}\\\footnotesize chaque transaction repérée dans le texte};
\node[step=houseC, text width=3.0cm] (hvis) at (-2.6,-4.9) {\textbf{OCR Claude Vision}\\\footnotesize redressée, lue en JSON};
\node[step=senateC, text width=3.0cm] (spar) at (2.6,-4.3) {\textbf{lecture des tableaux}\\\footnotesize colonnes par appariement flou};
\node[step=senateC, text width=3.0cm] (svis) at (6.0,-4.3) {\textbf{OCR Claude Vision}\\\footnotesize image déjà droite};
...
\node[file, text width=12.6cm, fill=houseC!10, draw=coreC, line width=1pt] (fin) at (0,-10.7)
  {\textbf{TABLE FINALE} — \co{06_\{ch\}_\{an\}_FINAL.csv} · 28 colonnes · \textbf{contrat 12 champs métier}};
...
{\footnotesize\emph{De haut en bas~: acquisition (deux sources) · tri puis extraction (quatre pistes, une par forme de document) · tronc commun partagé (identité → enrichissement → table finale → validation).}}
\end{center}
...
Une table « pleine » n'est pas pour autant une table \emph{saine}~: ses dates peuvent être incohérentes, les
délais légaux non respectés, son activité concentrée sur quelques déposants. \co{common/quality.py}
(\co{build_report}) y répond par \textbf{six contrôles} et quatre figures de contrôle
(figure~\ref{fig:qualite}), calculés en \textbf{lecture seule} des tables finales, sans aucun appel d'API
(matplotlib en mode \co{Agg})~:
...
\item \textbf{(f) Validation externe contre Quiver}, par scope — développée à la section suivante (§\ref{sec:quiver}), agrégée ici depuis les réconciliations figées (\co{07c}/\co{07g}/\co{07h}).
\end{itemize}
...
\begin{subfigure}[t]{0.48\textwidth}\centering
  ...
  \caption{Distribution de \co{amount_midpoint} (échelle \co{log10}).}
\end{subfigure}

\vspace{0.7em}
\begin{subfigure}[t]{0.48\textwidth}\centering
  \includegraphics[width=\linewidth]{quality/transactions_par_an.png}
  \caption{Transactions par année de transaction et par chambre.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}\centering
  \includegraphics[width=\linewidth]{quality/top_deposants.png}
  \caption{Top~12 déposants par nombre de transactions.}
\end{subfigure}
\caption{Les quatre contrôles graphiques de qualité, générés par \co{common/quality.py}
(lecture seule, aucun appel API)~; la domination des déposants-papier (Khanna, McCaul) est visible en (d).}
\label{fig:qualite}
\end{figure}

Le bilan global masque toutefois une dynamique. Au Sénat, la couverture en ticker chute à \textbf{45,4\,\%}
en 2025 et \textbf{43,7\,\%} en 2026, contre environ 80\,\% les années précédentes, le secteur suivant la
même pente. La cause n'est pas une dégradation de l'extraction mais une \textbf{composition d'actifs}~: la
part d'obligations municipales — non cotées, donc légitimement sans ticker — y augmente fortement. La
fiabilité des dates issues de l'OCR est, elle, suivie par la colonne \co{date_confidence}~: 95,3\,\% de
dates jugées plausibles à la Chambre, le creux de 2021 (79,6\,\%) tenant entièrement aux deux documents
manuscrits denses de Diana Harshbarger — une anomalie localisée, non systémique.

Cette colonne repose sur une fenêtre de plausibilité de \textbf{75~jours}~: le STOCK Act vise environ
45~jours, et l'on tolère cette marge pour absorber le bruit de lecture de l'OCR. C'est une heuristique
appliquée aux seules lignes OCR, non une vérité-terrain — ce rôle revient à Quiver, objet de la section
suivante.

\subsection{La donnée vue par sous-corpus}
\label{sec:corpus}
Les contrôles ci-dessus agrègent par chambre, mais la matière se répartit en réalité en \textbf{quatre
sous-corpus} — Chambre et Sénat, chacun en \emph{électronique} et en \emph{OCR papier} — dont la qualité
et la \emph{composition} diffèrent nettement. \co{common/quality.py} décline donc l'essentiel par ces
quatre familles (Chambre électronique 32\,676~; Chambre OCR 48\,970~; Sénat électronique 7\,161~; Sénat
OCR 1\,680).

\textbf{La couverture suit la composition d'actifs, pas la voie d'acquisition.} La part de ticker
renseigné passe de \textbf{88,6\,\%} (Chambre électronique) à \textbf{33,2\,\%} (Sénat OCR). Ce n'est pas
un défaut d'extraction~: le Sénat OCR est composé à \textbf{70\,\%} d'actifs « autres » non cotés et de
14\,\% d'actifs non identifiables, pour seulement \textbf{13\,\%} d'actions — c'est précisément cette
composition, et non le fait que ce soit du papier, qui explique que Quiver (qui ne suit que les actions)
« ne voie pas » ce corpus (§\ref{sec:quiver}). À l'inverse, les deux corpus de la Chambre sont à plus de
\textbf{81\,\%} d'actions.

\textbf{Le type de détenteur change radicalement d'un corpus à l'autre.} L'électronique de la Chambre est
surtout déclaré en compte propre (51,9\,\%) ou joint (26,2\,\%)~; l'OCR de la Chambre, lui, est dominé par
les comptes \emph{conjoint} (51,1\,\%) et \emph{enfant à charge} (35,2\,\%) — reflet des gros
déposants-papier (Khanna, McCaul, Harshbarger), qui déclarent l'essentiel via ces comptes.

\textbf{La concentration est extrême, et croît avec le papier.} Le volume déclaré se concentre sur une
poignée de déposants~: l'indice de Gini va de \textbf{0,88} (Chambre électronique, 234 déposants) à
\textbf{0,91} pour l'OCR de la Chambre (40 déposants, dont le top-10 capte \textbf{98,7\,\%} du volume),
tandis que le Sénat OCR ne compte que \textbf{5} déposants. Le tableau ci-dessous résume les quatre
familles~; le détail complet (couverture, qualité, mix, concentration, clusters de scan) est en
annexe~\ref{anx:metrics} (D.9–D.12).

\begin{center}\small
\begin{tabular}{l r rr r r rr}
\toprule
\bfseries Sous-corpus & \bfseries n & \bfseries ticker\,\% & \bfseries sect.\,\% & \bfseries date pl.\,\% & \bfseries médiane\,\$ & \bfseries Gini & \bfseries top-10\,\%\\
\midrule
{\color{houseC}Chambre électronique} & 32\,676 & 88,6 & 86,6 & — & 8\,000 & 0,88 & 69,0 \\
{\color{houseC}Chambre OCR} & 48\,970 & 83,1 & 81,0 & 95,3 & 8\,000 & 0,91 & 98,7 \\
{\color{senateC}Sénat électronique} & 7\,161 & 80,3 & 72,1 & 85,0 & 8\,000 & 0,84 & 85,0 \\
{\color{senateC}Sénat OCR} & 1\,680 & 33,2 & 19,4 & 98,5 & 32\,500 & 0,70 & 100,0 \\
\bottomrule
\end{tabular}
\end{center}
{\footnotesize \emph{date pl.\,\%} = part des dates OCR jugées plausibles (fenêtre 75~j)~; « — » pour la
Chambre électronique (pas d'OCR). \emph{top-10\,\%} = part du volume détenue par les 10 plus gros
déposants du corpus.}

\begin{figure}[htbp]\centering
\begin{subfigure}[t]{0.48\textwidth}\centering
  \includegraphics[width=\linewidth]{quality/mix_operations_par_corpus.png}
  \caption{Sens des opérations (achat / vente) par sous-corpus.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}\centering
  \includegraphics[width=\linewidth]{quality/mix_actifs_par_corpus.png}
  \caption{Mix de types d'actifs par sous-corpus.}
\end{subfigure}

\vspace{0.7em}
\begin{subfigure}[t]{0.48\textwidth}\centering
  \includegraphics[width=\linewidth]{quality/volume_par_secteur.png}
  \caption{Volume déclaré par secteur GICS (top 12).}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}\centering
  \includegraphics[width=\linewidth]{quality/concentration_lorenz.png}
  \caption{Concentration du volume par déposant (Lorenz, Gini).}
\end{subfigure}
\caption{Décomposition par sous-corpus, générée par \co{common/quality.py} (lecture seule)~: le Sénat OCR
est dominé par le non-coté (b) et le volume est fortement concentré (d).}
\label{fig:corpus}
\end{figure}
...
\toprule
\bfseries Sous-corpus & \bfseries n & \bfseries ticker\,\% & \bfseries sect.\,\% & \bfseries comm.\,\% & \bfseries date pl.\,\% & \bfseries cohér.\,\% & \bfseries mont.\,\%\\
\midrule
{\color{houseC}Chambre électronique} & 32\,676 & 88,6 & 86,6 & 74,7 & — & 99,9 & 100,0 \\
{\color{houseC}Chambre OCR} & 48\,970 & 83,1 & 81,0 & 94,5 & 95,3 & 99,7 & 98,2 \\
{\color{senateC}Sénat électronique} & 7\,161 & 80,3 & 72,1 & 58,5 & 85,0 & 100,0 & 100,0 \\
{\color{senateC}Sénat OCR} & 1\,680 & 33,2 & 19,4 & 96,1 & 98,5 & 99,8 & 99,5 \\
\bottomrule
\end{tabular}
\end{center}
{\footnotesize \emph{cohér.\,\%} = \co{disclosure ≥ transaction}~; \emph{date pl.\,\%} = dates OCR
plausibles (75~j~; « — » hors OCR)~; \emph{mont.\,\%} = montant renseigné. Identité (\co{bioguide_id}) et
ancienneté (\co{years_in_office}) = 100\,\% partout~; \co{amount_split_flag} partout \co{False}.}
