"""Convertit un rapport Markdown (ex. RAPPORT_DONNEES.md) en PDF imprimable A4.
...
Usage : `python -m common.report_pdf`  (→ RAPPORT_DONNEES.pdf)
...
def main():
    md = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent / "RAPPORT_DONNEES.md"
    pdf = build_pdf(md)
