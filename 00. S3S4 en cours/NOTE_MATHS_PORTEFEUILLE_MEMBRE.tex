...
\section{Excès et Information Ratio}
...
\textbf{Ajustement par le marché (}$\boldsymbol\beta$\textbf{).} L'excès brut $r_{\text{strat}}-r_{\text{SPY}}$
suppose $\beta=1$. On régresse plutôt les rendements quotidiens :
$$\boxed{\,r_{\text{strat}}(t)=\alpha+\beta\,r_{\text{SPY}}(t)+\varepsilon\,}$$
\begin{itemize}
  \item $\beta$ = exposition au marché (levier de \emph{style}) ;
  \item $\alpha$ = rendement en plus du marché \emph{à exposition égale} (le \emph{talent}) ;
        annualisé $\alpha_{\text{ann}}=(1+\alpha)^{252}-1$ ;
  \item significativité par le $t$ de la régression $t_\alpha=\alpha/\mathrm{SE}(\alpha)$
        (OLS ; un écart-type Newey-West élargirait $\mathrm{SE}$, l'autocorrélation quotidienne étant réelle).
\end{itemize}
