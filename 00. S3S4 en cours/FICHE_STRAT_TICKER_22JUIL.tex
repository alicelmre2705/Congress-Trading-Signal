...
% ═══════════ Produits réels : NANC / GOP ═══════════
\et{}{Les produits réels : réplique NANC / GOP}
\textbf{Ce qu'ils font} :
\begin{pts}
\item \textbf{NANC} (élus Démocrates) et \textbf{GOP} (Républicains) sont les ETF qui copient le Congrès. Règle du prospectus : sur \textbf{3 ans glissants}, achats \textbf{nettés} des ventes, \textbf{pondérés au montant} (\$), 100--200 titres, à la \textbf{divulgation}.
\item On réplique fidèlement leur règle et on compare à SPY.
\end{pts}
\[ w^{\text{NANC}}_{i,t} \;\propto\; \Big[\;\textstyle\sum_{k\,\in\,\text{parti},\ t-3\text{ans}<\delta_k\le t} A_k\,(\mathbf{1}_{achat}-\mathbf{1}_{vente})\;\Big]^{+} \]
\begin{center}\scriptsize
\begin{tabular}{@{}l r r r r r@{}}
\toprule
& excès/an & $t$ & $\alpha$/an & $\beta$ & NAV \\
\midrule
réplique NANC (Dém.) & $+0{,}23\,\%$ & 0,18 & $-0{,}81\,\%$ & 1,05 & 527 \\
réplique GOP (Rép.) & $+0{,}67\,\%$ & 0,22 & $-0{,}29\,\%$ & 1,02 & 549 \\
\bottomrule
\end{tabular}
\end{center}
\fig{nanc_gop.png}{}
\textbf{Ce qu'on observe} :
\begin{pts}
\item Notre réplique reproduit leurs holdings : \textbf{NVDA} 25 \%, AVGO, META, GOOG, PLTR\dots{} = méga-cap tech.
\item $\beta \approx 1$, $\alpha \approx 0$ ; NAV 527/549 colle à SPY (517).
\item \emph{Excès} $\ne$ \emph{total} : le tableau donne l'excès sur SPY ($+0{,}2\,\%$). Le \textbf{rendement total} de la réplique $\approx$ \textbf{14 \%/an} (2014-26) $=$ SPY ; sur 2023-2026 seul (la fenêtre de NANC) $\approx$ 19 \%/an --- comme leurs $\sim$22 \% (SPY $\sim$21 \% sur la même période). Leur perf = le rally méga-cap tech, pas un edge.
\end{pts}
