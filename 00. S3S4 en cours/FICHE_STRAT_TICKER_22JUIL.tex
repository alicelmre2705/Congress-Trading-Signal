...
\textbf{1. NANC / GOP} (ETF Subversive). \textbf{Ce qu'ils disent faire} \emph{(prospectus, p.2)} :
\begin{pts}
...
\item Pondéré au « \textbf{niveau de trading} » : gros achats \textbf{+ récurrents + de plusieurs élus} $\to$ surpondérés. \textbf{Les ventes de positions détenues avant la fenêtre ne comptent pas.}
\end{pts}
\[ \begin{gathered} w^{\text{NANC}}_{i,t} \;\propto\; B_{i,t}\times m_{i,t}\,,\qquad \text{puis plafonné à } 8{,}5\,\% \\[1pt] B_{i,t}=\!\!\sum_{k\,:\,\text{achat},\ t-3\text{ans}<\delta_k\le t}\!\! A_k\ ,\qquad m_{i,t}=\#\{\text{élus acheteurs de }i\} \end{gathered} \]
\begin{center}\scriptsize
\begin{tabular}{@{}l r r@{}}
\toprule
...
\midrule
NANC (Dém.) & $+2{,}98\,\%$ & 1,47 & $+1{,}26\,\%$ & 1,08 & 672 \\
GOP (Rép.) & $+1{,}24\,\%$ & 1,62 & $+1{,}12\,\%$ & 1,00 & 612 \\
\bottomrule
\end{tabular}
\end{center}
\fig{nanc_gop.png}{}
\textbf{Ce qu'on observe} :
\begin{pts}
\item \textbf{Correction} (liste réelle NANC, 24/07) : notre 1re version pondérait au \textbf{net brut} (achats$-$\emph{toutes} ventes) $\to$ elle \textbf{excluait} MSFT/AAPL/AMZN (élus vendant de vieilles positions) $\Rightarrow$ recouvrement \textbf{2/10}. Corrigée (achats $\times$ nb d'élus, cap 8,5 \%) : \textbf{5/10}, \textbf{NVDA 8,5 \% = exact}, GOOG/MSFT/AMZN/AAPL présents.
\item On retrouve aussi leur \textbf{rendement} : excès $+2{,}98\,\%$/an (le vrai NANC bat le S\&P de $\sim+2{,}7\,\%$/an depuis sa création) --- mais $\beta$ 1,08 (tilt tech) et $t$ 1,5 : \textbf{modeste, pas franchement significatif}. \emph{Résidu :} AMAT/CRWD\dots{} quasi absents de \emph{notre} donnée (source $\ne$ la leur) ; formule exacte non divulguée.
\end{pts}

\textbf{2. Quiver Quantitative} (indices \og{}Congress Trading\fg{}). \textbf{Ce qu'ils disent faire} :
\begin{pts}
...
\end{center}
\fig{quiver.png}{}
\textbf{Ce qu'on observe} --- ils affichent \textbf{CAGR 36,2 \%} ($+604\,\%$), nous $\sim$22 \%. On a cherché l'erreur chez nous :
\begin{pts}
\item \textbf{On matche tout ce qu'ils divulguent} et rien ne bouge : long, pondéré au \$, \textbf{rebalancement hebdo} (comme eux) $=22{,}3\,\%$ ; le vrai équipondéré 1/N fait \emph{moins} (NAV 412 $<$ 565) ; ETF écartés $=2{,}5\,\%$ (VTI/SPY) ; 296 titres non cotés $=6{,}5\,\%$ (fusions, même $\times2\Rightarrow+6{,}5\,\%$). Toute construction défendable $\Rightarrow \beta\approx1$, $\sim$22 \% $=$ SPY. L'écart au 36 \% est dans ce qu'ils \textbf{ne publient pas} (règle de sortie).
\item \textbf{Leur 36 \% exigerait $+12\,\%$ d'alpha/an} qu'on ne trouve pas : leur $\beta$ 1,14 sans alpha $\Rightarrow$ CAPM $\sim$24 \%/an ; et leur vol affichée $4{,}63\,\%$ est \textbf{incohérente} avec $\beta$ 1,14 (qui impose $\ge19\,\%$). \emph{(Leur Sharpe, lui, n'exclut pas 36 \% : on ne l'invoque pas.)}
\item \textbf{Le Long-Short $\approx$ marché levé} : les deux jambes ($\sim$400 titres chacune) collent au marché ($\beta$ 1,04). Le $+0{,}92\,\%$/an (t 0,60) n'est pas significatif et \textbf{dépend du panier short que Quiver ne divulgue pas} ($+1{,}37\,\%$ avec un short plus strict).
\end{pts}

% ═══════════ É11 ═══════════
\et{É11}{Les validations (le moteur calcule juste)}
\begin{center}\scriptsize
\begin{tabular}{@{}l l@{}}
\toprule
$\sum_i w_{i,t} = 1$ à chaque coupe & \textbf{1,0000000000} \\
$V(t)$ recalculée depuis les parts & écart \textbf{0,00e$+$00} \\
tranches FIFO recalculées hors moteur & identiques \\
régimes limites de la pondération & $\checkmark$ \\
\bottomrule
\end{tabular}
\end{center}
\lire{chaque ligne = un contrôle du moteur, à droite le résultat obtenu. $V(t)$ = valeur du portefeuille ; \emph{parts} = nombre d'actions détenues.}

\et{}{Limites assumées}
\begin{pts}
\item Rendements \textbf{bruts}, sans coûts de transaction.
\item $\tau$ (Tests 1--4) = info \emph{avant} publication : borne haute, non réplicable telle quelle --- d'où le Test 5 à $\delta$.
\item Univers filtré sur la qualité des prix sur \textbf{toute} la période (pas strictement point-in-time) ; $W{=}42$ fixé plein-échantillon.
\end{pts}
