...
\begin{pts}
\item Univers = 100--200 titres tradés par les élus \textbf{Démocrates} (NANC) / \textbf{Républicains} (GOP) via les PTR, sur \textbf{3 ans}.
\item Pondéré au \og{}\textbf{niveau de trading}\fg{} \emph{(achats nets des ventes récentes)} : gros achats \textbf{+ récurrents + de plusieurs élus} $\to$ surpondérés ; petits \textbf{+ ventes récentes + one-off} $\to$ sous-pondérés.
\item \textbf{Les ventes de positions détenues avant la fenêtre / avant l'entrée en fonction ne comptent pas.}
\end{pts}
\textbf{Ce qu'on fait} --- chaque fin de mois $t$, pour le parti $P$ :
\begin{pts}
\item \textbf{Fenêtre} : les \textbf{achats} des élus de $P$ divulgués sur $(t-3\,\text{ans},\,t]$. \emph{Simplification} : on ignore \textbf{toutes} les ventes (on ne nette pas les ventes récentes --- cf. annexe).
\item \textbf{Score} : $s_i = B_i\times m_i$, avec $B_i=\sum_k A_k$ (\$ acheté, midpoint --- capte \emph{gros}\,$+$\,\emph{récurrents}) et $m_i=\#\{$élus acheteurs$\}$ (capte \emph{plusieurs élus}).
...
\end{pts}
\[ w^{P}_{i,t} \;\propto\; s_{i,t}=B_{i,t}\times m_{i,t}\quad(\text{plafonné } 8{,}5\,\%,\ P=\text{NANC/GOP}) \]

\textbf{Ce que eux ont vs nous} (top-15 holdings) :
\fig{nanc_compare.png}{barres : vrai NANC (gris) vs réplique (bleu). Nos 5 plus grosses lignes (NVDA, GOOG, AMZN, MSFT, AAPL) sont toutes de vraies têtes de NANC ; NVDA à 8,5 \% \emph{(valeur fixée par le cap)} ; AMAT, CRWD, PM, AXP sont quasi \textbf{absents de notre donnée}.}
...
\end{center}
\fig{nanc_gop.png}{}
\fig{nanc_rolling.png}{$\beta$ et $\alpha$ glissants (OLS 252 j, validé $=$ \texttt{polyfit}) : $\beta$ colle à $\sim$1 (tilt marché) ; l'$\alpha$ oscille autour de 0 et traverse zéro sans cesse --- pas d'edge persistant.}
\obs{On retrouve leur \textbf{rendement} : le vrai NANC bat le S\&P de $\sim+2{,}7\,\%$/an depuis sa création $\approx$ notre $+2{,}98\,\%$.}

\smallskip
\textbf{Variante 2 --- une autre lecture du \og{}netting\fg{}.} Le prospectus nette les achats contre les
\emph{ventes récentes}, mais \textbf{pas} les ventes de vieilles positions. On le fait proprement avec la \textbf{purge
$\gamma$} (même FIFO plein historique que É2 : une vente consomme les \emph{plus vieux} lots d'abord) au seuil
\textbf{3 ans} : $\gamma_k$ $=$ part de la vente $k$ portant sur du récent ($\le$3 ans) ; une vente de lot dormant a
$\gamma_k\!\to\!0$ (ignorée). Net par (élu, titre) $\mathrm{net}=[\sum A - \sum A\gamma]^{+}$ ; score $s_i=\mathrm{net}_i\times m_i$,
\textbf{de-minimis} 1 \%, \textbf{100--200} titres.
\begin{center}\scriptsize
\begin{tabular}{@{}l r r r r@{}}
\toprule
V1 vs V2 \emph{(2014-26)} & exc./an & $\alpha$/an & NAV & recouv. \\
\midrule
NANC $\cdot$ V1 (achats)   & $+2{,}98\,\%$ & $+1{,}26\,\%$ & \textbf{672} & 5/10 \\
NANC $\cdot$ V2 (net $\gamma$) & $+1{,}40\,\%$ & $-0{,}50\,\%$ & \textbf{558} & 5/10 \\
GOP $\cdot$ V1 (achats)    & $+1{,}24\,\%$ & $+1{,}12\,\%$ & \textbf{612} & --- \\
GOP $\cdot$ V2 (net $\gamma$)  & $+1{,}34\,\%$ & $+1{,}15\,\%$ & \textbf{617} & --- \\
\bottomrule
\end{tabular}
\end{center}
\obs{Le netting \textbf{ne change pas les têtes} (ventes de vieux lots ignorées, top-15 même \textbf{9/15}) mais
\textbf{ronge la perf de NANC} : son $+2{,}98\,\%$ (V1) était en partie l'artefact du \og{}on ignore les ventes\fg{}
--- une fois nettées, NANC tombe à $+1{,}40\,\%$ ($\alpha$ négatif), GOP ne bouge quasi pas. C'est la lecture
\textbf{la plus fidèle} du prospectus. Sans le cap, le score exploserait (NVDA 37 \%, $N_{\mathrm{eff}}$ 6 vs 33 réel).
\emph{Recouv.\ sur NANC seul : holdings réels GOP non fournis.}}

\smallskip
\textbf{Variante 3 --- le livre du fonds (leur vraie mécanique).} Le prospectus ne décrit pas un score recalculé
chaque mois mais un \textbf{livre événementiel} : \og{}\emph{the Fund will buy or sell a security \textbf{when} a
position is reported}\fg{}. Livre initial à la création ($t_0 = $ 07/02/2023) $=$ net $\gamma'$ des 3 ans précédents
(en parts au prix de $t_0$) ; puis chaque achat divulgué \textbf{ajoute} $A_k/P(\delta_k)$ parts, chaque vente
\textbf{retire} $\gamma'_k A_k/P(\delta_k)$ ($\gamma'$ : non-apparié $=$ vieux, ignoré) ; entre les événements les
poids \textbf{dérivent avec les prix} --- ni rebalancement, ni cap.
\begin{center}\scriptsize
\begin{tabular}{@{}l c c r r@{}}
\toprule
fidélité aux holdings réels & top-10 & top-15 & $\rho$ & expo comm. \\
\midrule
NANC $\cdot$ V1 (score achats)   & 5/10 & 7/15 & 0,73 & 51 \% \\
NANC $\cdot$ V2 (score net $\gamma$) & 5/10 & 9/15 & 0,67 & 48 \% \\
NANC $\cdot$ V3 (livre du fonds) & 5/10 & 8/15 & \textbf{0,74} & 46 \% \\
GOP $\cdot$ V2 (score net $\gamma$)  & 3/10 & 4/15 & 0,11 & 27 \% \\
GOP $\cdot$ V3 (livre du fonds)  & 0/10 & 0/15 & 0,16 & 26 \% \\
\bottomrule
\end{tabular}
\end{center}
\fig{nanc_gop_v3.png}{réel (gris) vs V3 : \textbf{NVDA émerge à 8,9 \% tout seul} (réel 8,5, sans aucun cap --- la
mécanique s'auto-régule) ; mais AMAT reste à 0,3 \% (réel 5,2) et FIX à 0,5 \% (réel 8,4) : leurs flux ne sont pas
dans notre donnée à cette taille.}
\obs{\textbf{Les trois lectures de la règle plafonnent au même endroit} ($\rho\!\sim\!0{,}7$, expo $\sim$50 \% pour
NANC ; GOP irrécupérable). Le résidu n'est \textbf{plus la règle} : (1) leur \emph{Data Provider} $\ne$ nos montants
midpoint de fourchettes PTR (AMAT : 0,98 M\$ de flux chez nous, intenable à 5,2 \% du fonds) ; (2) la
\textbf{discrétion} explicite de l'adviser (exclusions, sous-pondérations) ; (3) trous nommés (FIX $=$ 1 élu chez
nous ; SpaceX/IBIT hors univers). \emph{Même stratégie, données différentes $\Rightarrow$ portefeuilles différents.}}
\obs{\textbf{Contre-épreuve --- audit et réparation de NOTRE chaîne} : 10 séries de prix récupérées (renommages
MMC$\to$MRSH, FI$\to$FISV\ldots), \textbf{7 287 transactions OCR réintégrées} (892 PTR papier écartés par le
filtre anti-manuscrit, dont 6 482 républicaines --- McCaul 2015-16) et sensibilité des montants ouverts
(\og{}over \$1M\fg{} plancher$\to$3 M) : \textbf{la fidélité ne bouge pas} ($\rho$ identiques ; la variante 3 M
\emph{dégrade} GOP). Même les PTR récupérés ne contiennent \textbf{aucun flux à l'échelle du FIX 8,4 \%} du vrai
GOP $\Rightarrow$ l'écart résiduel est \textbf{prouvé} côté fournisseur/discrétion, pas dans notre chaîne.}
