...
{\footnotesize
\textbf{En quatre blocs} : \textbf{I} ce qu'on fait, en onze étapes --- \textbf{II} ce que ça donne ---
\textbf{III} la version qui n'achète que des ETF --- \textbf{IV} comment on l'a vérifié.\par
\vspace{0.5mm}
...
% ══════════════════════════════════════════════════════════════════════
\mouv{I --- Ce qu'on fait}{onze étapes, dans l'ordre d'exécution}
...
\item \textbf{la pondération} --- $w \propto s_i$ (M3), $\propto B_i$, $\propto m_i$, ou $1/N$.
      $\mathcal S$ reste défini par le score : c'est une décomposition, pas une autre sélection ;
\item \textbf{le filtre $\theta$} --- écarter les opérations d'un dépôt de $\theta$ opérations ou plus, même
      élu, même jour. M3 : $\theta = \infty$.
\end{pts}
...
\item le chargement SMB de M3-NANC ($-0{,}131$) est \emph{du même ordre} que celui du SPY ($-0{,}123$) : le
      portefeuille n'a pas de style propre.
\end{pts}

\et{c}{Le coût, et faut-il ralentir}
...
\vspace{0.4mm}
\begin{pts}
\item \textbf{le frein n'est pas retenu.} Règle écrite d'avance : on ne le garde que s'il améliore les
      \emph{deux} partis. Un seul plafond y parvient ($+0{,}03$ à 5~\%/mois) et il est encadré de deux qui
      échouent ($-0{,}05$ à 10~\%, $-0{,}48$ à 2~\%) --- profil de bruit, pas d'une contrainte qui mord.
\end{pts}

\et{d}{Le filtre anti-bruit $\theta$}
...
\et{e}{Deux variantes testées, non retenues}
\begin{pts}
\item \textbf{netter les ventes} (comme le fonds) --- en soustrayant la part de chaque vente portant sur un
      achat de moins de trois ans (FIFO), l'excès NANC \textbf{tombe} de $+3{,}40$ à $+1{,}31$~\%/an et
      l'$\alpha_1$ passe à $-0{,}54$ ;
\item \textbf{les 150 titres} : la sélection mord réellement --- $\lvert\mathcal S\rvert$ vaut 150 en médiane,
      99 au minimum côté NANC.
\end{pts}

% ══════════════════════════════════════════════════════════════════════
\mouv{III --- Version 2}{un portefeuille qui n'achète que des ETF}

\textbf{La méthode ne change pas} : M3 est calculée en entier, à l'identique. Ce qui change vient
\emph{après}.

\et{a}{Le passage aux ETF, en trois gestes}
\begin{pts}
\item \textbf{calculer M3 en entier} --- score, top-150, plafond 10~\% \emph{par titre} ;
\item \textbf{remplacer chaque ligne par l'ETF de son secteur.} Le rattachement titre $\to$ GICS $\to$ SPDR
      est un \textbf{intrant} du dépôt, renseigné sur 100~\% des lignes. Clé = ticker \emph{après} fusion ;
      cinq tickers portent deux secteurs (BALL, CPAY, SONY, TFC, XYZ), tranchés par le majoritaire ;
\item \textbf{sommer les poids}, puis \textbf{renormaliser} sur les seuls secteurs cotés à la date.
\end{pts}
\begin{pts}
\item \emph{conséquence} : le plafond de 10~\% \textbf{ne survit pas} à l'agrégation. Il porte sur les
      titres ; cinq titres de technologie à 6--10~\% chacun font un secteur à 46,6~\%. \textbf{Il n'y a donc
      aucun plafond sectoriel} --- la plus grosse ligne pèse 46,6~\% (démocrates) et 42,2~\% (républicains) ;
\item \emph{conséquence} : deux des onze ETF n'existent pas au début du backtest --- \textbf{XLRE} cote
      depuis 10/2015, \textbf{XLC} depuis 06/2018. Sur ces coupes-là, les titres qui devaient y aller
      (Google, Meta, l'immobilier) n'ont pas d'instrument : leur poids est réparti sur les autres secteurs,
      \textbf{jusqu'à 15,4~\% du portefeuille}. Sur 2014--2018 on ne tient donc pas vraiment l'exposition
      sectorielle de M3, mais une version déformée --- d'où la fenêtre \textbf{2019--2026}, où les onze
      cotent tous.
\end{pts}

\et{b}{Le portefeuille unique}

Retenir le camp démocrate \emph{ou} républicain serait un choix fait après avoir vu les résultats. On calcule
donc le score sur \textbf{tous les élus}. Ce n'est pas la moyenne des deux : un titre acheté par trois
démocrates \emph{et} trois républicains obtient $m_i = 6$ au lieu de 3 --- \textbf{le consensus bipartisan
est récompensé}. À la dernière coupe : 39 acheteurs démocrates, 60 républicains, \textbf{100 fusionnés}.
...
$\lambda = 0$ donne le marché, $\lambda = 1$ la version sectorielle simple. Poids du marché reconstruits
depuis les prix seuls, avec ceux de \textbf{l'année précédente} --- aucun regard vers l'avenir.
...
\begin{pts}
\item \textbf{à $\lambda = 2$ le portefeuille unique domine les deux camps sur les quatre colonnes} :
      $+3{,}41$~\%/an net contre $+3{,}17$ et $+3{,}21$, pour un écart au marché \emph{plus faible} (4,2 contre
      4,6 et 5,6~\%), un meilleur rendement par unité d'écart (0,83 contre 0,70 et 0,59) et \emph{moins} de
      rotation (41 contre 43 et 54~\%) ;
\item \textbf{on ne choisit pas $\lambda$, on choisit l'écart au marché} --- $\lambda$ s'en déduit. Le
      rendement par unité d'écart plafonne à 0,83--0,85 vers $\lambda = 2$--3, puis cesse de progresser ;
\item \textbf{une part de l'excès vient du $\beta$, pas de l'$\alpha$} : à $\lambda = 2$, l'excès vaut
      $+3{,}41$ mais l'$\alpha_1$ seulement $+1{,}29$ --- le portefeuille est plus exposé au marché que
      l'indice ($\beta$ 1,09). Le reste, $\simeq 2$ points, est le prix de ce risque en plus ;
\item \textbf{et $\lambda$ concentre.} \emph{Il n'y a aucun plafond au niveau du secteur} --- celui de 10~\%
      porte sur les titres et ne survit pas à l'agrégation. Le plus gros secteur atteint \textbf{38,7~\% à
      $\lambda = 0$} (c'est la concentration du marché lui-même, en 2025), \textbf{47,8~\% à $\lambda = 1$}
      et \textbf{61,2~\% à $\lambda = 2$} --- $N_{\text{eff}}$ tombant de 6,5 à 4,6. C'est la contrainte qui
      décidera sans doute avant le tracking error.
\end{pts}

\et{d}{Deux pistes écartées}
\begin{pts}
\item \textbf{32 instruments plus fins} (semi-conducteurs, biotechnologie, défense, banques régionales) :
      ...
      le signal : \textbf{les ETF fins ne battent le secteur qu'ils découpent que 6 fois sur 21}, écart moyen
      $-1{,}6$~pt/an ;
\item \textbf{recomposer plus souvent} : sans objet. Le signal est une \textbf{inclinaison}, pas une rotation
      --- 85~\% des écarts de poids sont permanents côté démocrate, 76~\% côté républicain, et 2,2~\% du
      portefeuille seulement change de main d'une coupe à l'autre.
\end{pts}
\begin{pts}
\item \emph{mesuré hors de tout portefeuille} : sur 3\,112 observations (148 coupes $\times$ 11 secteurs
      $\times$ 2 camps), un secteur surpondéré de 10 points sur-performe les autres de \textbf{0,3 à 0,4~\%
      le mois suivant}. Le plus gros contributeur ne pèse que 36~\% de l'excès côté démocrate (la
      technologie) et 34~\% côté républicain (l'énergie) --- deux moteurs \emph{opposés}.
\end{pts}
