...
sous-fenêtre, un seul camp, parmi 84 runs (annexe D).}\par

\etape{13}{La forme livrable : le marché, plus un tilt \emph{(2020--2026 seulement)}}
\[ w \;=\; \underbrace{w^{\text{marché}}}_{\text{la brique de base}} \;+\; \lambda\,\underbrace{(w^{\text{signal}} - w^{\text{marché}})}_{\text{le pari}} \]
$\lambda = 0$ donne le marché \emph{reconstruit} --- onze SPDR rebalancés chaque mois, d'où $-0{,}33$~\%/an
et 2,1~\% d'écart ; $\lambda = 1$ donne la version sectorielle simple. Les deux grandeurs qui décident
...
\[ r^{\text{SPY}} = \beta_1 r^{\text{XLK}} + \beta_2 r^{\text{XLF}} + \dots + \beta_{11} r^{\text{XLU}}
   + \varepsilon \]
Sous $\sum \beta = 1$ et $\beta \ge 0$, les $\beta$ servent de \emph{proxy} des poids sectoriels, estimés sur
\textbf{l'année précédente} --- aucun regard vers l'avenir. Le $R^2$ (0,984 à 0,999) atteste que les onze
SPDR \textbf{engendrent} l'indice ; il n'identifie pas chaque coefficient. Après le tilt, tout poids
négatif est ramené à zéro (long seul).

{\centering\scriptsize\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{@{}l r r r r r@{}}
\toprule
$\lambda$ & excès \textbf{net}/an & $\alpha_1$ \emph{(t)} & $\beta$ & \textbf{écart marché} & rdt/écart \\
\midrule
0 \emph{(le marché)} & $-0{,}33$ & $-0{,}65$ \emph{(--0,8)} & 1,00 & 2,1~\% & --- \\
1 & $+1{,}73$ & $+0{,}42$ \emph{(0,45)} & 1,04 & 2,6~\% & 0,68 \\
2 & $\mathbf{+3{,}41}$ & $\mathbf{+1{,}29}$ \emph{(0,87)} & 1,09 & \textbf{4,2~\%} & \textbf{0,83} \\
3 & $+4{,}31$ & $+1{,}71$ \emph{(0,93)} & 1,10 & 5,2~\% & 0,85 \\
\bottomrule
\end{tabular}\par}
\vspace{0.2mm}
{\scriptsize\color{grisf} l'\textbf{écart au marché} = écart-type annualisé de la différence avec l'indice : à
4,2~\%, deux années sur trois l'écart au SPY se tient à $\pm 4{,}2$~\% \emph{autour de sa moyenne}, non autour de zéro. \textbf{Fenêtre 2020--2026,
net de 10~pb --- ne se compare pas au tableau ci-dessus}, qui couvre 148 coupes depuis 2014, brut de frais.
\textbf{Seule la colonne d'excès est nette} : $\alpha_1$, $\beta$ et rdt/écart sont mesurés sur la série brute.}\par
\vspace{0.5mm}
...
\item \textbf{et le tilt concentre} : le plus gros secteur pèse 38,7~\% à $\lambda = 0$ \emph{(la concentration du marché \textbf{reconstruit} : les
      SPDR plafonnant leurs lignes, la régression surcharge XLK --- la part réelle de la tech est plus basse)},
      47,8~\% à $\lambda = 1$, \textbf{61,2~\% à $\lambda = 2$}
      --- $N_{\text{eff}} = 1/\sum w_i^2$ de 6,5 à 4,6.
\end{pts}
...
\emph{isolé} : ce sont deux cases parmi les 84 ci-dessous, et la première est une sous-fenêtre choisie après
coup sur un seul camp. Nous ne les revendiquons pas.\par
\vspace{0.5mm}
\textbf{La multiplicité.} \textbf{84 runs distincts} passent au bilan dans le notebook, et tous y sont publiés ;
