...
\item \textbf{la carte est datée} --- deux secteurs n'existaient pas au début :
\end{pts}
\vspace{-0.6mm}
{\centering\scriptsize\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}l l l@{}}
\toprule
jusqu'au & le secteur d'époque & l'ETF \\
\midrule
16/09/2016 & l'immobilier est dans Financials & \textbf{XLF} \\
21/09/2018 & la communication est dans la technologie & \textbf{XLK} \\
 & \dots ou dans la consommation & \textbf{XLY} \\
\bottomrule
\end{tabular}\par}
\vspace{0.3mm}
{\scriptsize\color{grisf} dates de \textbf{bascule des indices}, pas de cotation des ETF ; le discriminant
entre XLK et XLY est l'industrie du titre.}\par
...
deux plus hauts $t$ du document (1,93 et \textbf{2,05}), \textbf{aucun n'atteint le seuil} --- voir \textbf{e}.}\par
...
net de 10~pb --- ne se compare pas au tableau ci-dessus} (voir \textbf{e}).}\par
\vspace{0.5mm}
...
\et{c}{Le signal sectoriel existe --- mesuré hors de tout portefeuille}
...
\et{e}{Comment lire ces chiffres}
\begin{pts}
\item \textbf{les deux tableaux ne se comparent pas} : 148 coupes depuis le 28/02/2014 \emph{brut}, contre
      \textbf{77 depuis le 31/01/2020} \emph{net} de 10~pb --- le tilt exige les poids du marché de l'année
      précédente, or les onze SPDR ne cotent tous qu'à partir de 2019 ;
\item pourtant \textbf{$\lambda = 1$ \emph{est} la version sectorielle simple} : $w^{\text{mkt}} +
      1\cdot(w^{\text{signal}} - w^{\text{mkt}}) = w^{\text{signal}}$. L'écart entre $+1{,}73$ et $+1{,}15$ est
      \textbf{entièrement} fenêtre et frais --- sur 2019+ les deux valent $+1{,}70$ et $+1{,}78$ ;
\item \textbf{rien n'est significatif} : à treize années il faudrait $\lvert t\rvert > 2{,}18$. Les deux plus
      hauts $t$ du document, 1,93 et \textbf{2,05}, restent dessous --- IC 95~\% du portefeuille unique
      $[-0{,}3;+5{,}4]$ sur les titres, tous deux contiennent zéro ;
\item \textbf{ce que la carte datée a réparé} : un titre sans ETF était \emph{écarté}, soit \textbf{11 titres
      du top-150 en médiane} et 15,2~\% du portefeuille (jusqu'à 20,8) sur 52 coupes $\to$ désormais
      \textbf{0 titre, 0,0~\%} ;
\item \emph{en regard}, avec la carte d'aujourd'hui les trois lignes ETF rendaient $+1{,}50$, $+0{,}45$ et
      $+1{,}15$ au lieu de $+1{,}78$, $+0{,}78$ et $+1{,}45$ ;
\item \textbf{et 2019--2026 n'a pas bougé d'un millième} : aucune de ses coupes n'avait de secteur manquant, le
      gain porte donc sur 2014--2018 seul. Restent \textbf{huit tickers} non attribués --- des \emph{fonds},
      pas des actions (annexe C).
\end{pts}
...
« \emph{Education Realty Trust} », un REIT, et son secteur Communication Services est simplement \emph{faux}.
