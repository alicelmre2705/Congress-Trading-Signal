...
\S16 à \S16.5 : les quatre méthodes (runs déclarés avant exécution), les quatre facteurs (validés sur le SPY :
$\beta_{\mathrm M}=0{,}980$, $\alpha=+0{,}04$~\%/an, \path{cache/ff_factors.csv}), les $\alpha/\beta$ glissants,
la rotation et les frais, puis l'\textbf{audit de méthode} : M3 y est réimplémentée de zéro d'après le seul texte
ci-dessus, et l'on y écarte le regard vers le futur, le fractionnement non retraité et la position gelée. L'étude
de fidélité au fonds NANC, dont M4 reprend le filtre, fait l'objet de \path{FICHE_NANC_GOP.pdf}.\par}
\end{multicols}
