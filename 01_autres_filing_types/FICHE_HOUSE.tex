...
\end{center}
\vspace{0.6mm}
{\footnotesize\textbf{Les documents, une fois pour toutes.} Un membre dépose \textbf{cinq types} de
documents ; deux \textbf{sections} comptent --- \textcolor{bleu}{\textbf{Schedule A}} = ce qu'il
\textbf{possède} (positions), \textcolor{orange}{\textbf{Schedule B}} = ce qu'il a \textbf{échangé}
(transactions).}
\begin{center}\includegraphics[width=0.62\linewidth]{figures_house/fig_10_catalogue.png}\end{center}
\vspace{-0.5mm}
{\footnotesize Le \textbf{B (transactions) n'existe que dans P, O, T} ; H et C ne portent que le A.
L'amendement (\textbf{A}) reprend les sections du document qu'il corrige. On \textbf{reconstruit} le
portefeuille de chacun avec les transactions, on l'\textbf{ancre et le vérifie} avec les positions, puis
une stratégie le \textbf{copie}.}
\end{tcolorbox}
...
\reponse{Les \textbf{900 membres} et leurs 4 trajectoires viennent du \textbf{registre officiel des élus}
(projet \emph{congress-legislators}), pas de nos dépôts (haut). \textbf{408} tradent des actions cotées,
vus selon le \textbf{type de document} qui les révèle (bas).}
\phare{408 sur 900}{membres qui tradent des actions cotées}{bleu}
\vis{fig_10_quadrant.png}
\vis{fig_10_trois_zones.png}
\src{305 vus par les PTR (dont 199 aussi par un rapport annuel), 103 vus \emph{seulement} par un rapport
annuel.\\
source : registre officiel \texttt{congress-legislators} (les 900 $+$ leurs trajectoires) ; les 408 qui
tradent $=$ PTR $\cup$ Schedule B, en actions cotées valorisables.}
\end{etapebox}}{\begin{etapebox}{2}{bleu}{2 $\cdot$ A-T-ON CE QUI EST DÛ ? --- attendu vs observé}
\reponse{Un \textbf{arrivé} doit une photo d'entrée, un \textbf{partant} une photo de sortie (plein $=$ observé,
pointillés $=$ attendu). Et chacun, un rapport chaque année.}
\phare{85 \%}{des années de mandat couvertes par un rapport (médiane)}{bleu}
\vis{fig_recup_photos.png}
\vis{fig_10_etape_sortie.png}
\src{Les scans sont bien océrisés (l'étiquette \og{} hors périmètre \fg{} du registre est périmée) :
\textbf{aucun manque ne vient d'un document \og{} pas exploité \fg{}}. Les membres sans photo ne détiennent
\emph{aucune action cotée} (immobilier, fonds, banque, assurance-vie) ou n'ont rien déposé --- \textbf{aucune
photo d'entrée d'un membre détenant une action cotée lisible} n'échappe au pipeline. Source : notebook
\emph{Récupération photos d'entrée}.}
\end{etapebox}}
\vspace{2mm}
\rangee{\begin{etapebox}{3}{bleu}{3 $\cdot$ L'INVENTAIRE --- deux schedules, deux métiers}
\reponse{Deux sections, deux métiers qui n'ont \textbf{rien à voir}. Le \textbf{Schedule A} (positions) sert à
\textbf{reconstruire le portefeuille} ; le \textbf{Schedule B} (transactions) se \textbf{compare aux PTR} pour
ne garder que les ordres inédits.}
\phare{3 sur 4}{lignes du Schedule B redisent déjà un PTR --- 26\,115 inédites}{bleu}
\vis{fig_inventaire_ab.png}
\pts{
\item \textbf{Schedule A} : \textbf{609\,886} positions dans 14\,887 documents (C, H, O, T) --- la matière
première du portefeuille (boîtes 4 et suivantes).
\item \textbf{Schedule B} : \textbf{110\,147} transactions (O, T), \textbf{comparées aux PTR} --- 84\,032
(76 \%) redisent un ordre déjà déclaré, seules \textbf{26\,115} sont inédites et complètent les 161\,076
déclarations rapides.
}
\src{source : cellules \og inventaire documentaire \fg{} et \og flux O/A/T net \fg{} (colonne \texttt{echo\_ptr}).}
...
\pts{
\item \textbf{107\,976} déclarations rapides (PTR, publiques en 27 jours) $+$ les \textbf{inédites} du
Schedule B qu'on a pu valoriser : \textbf{14\,084} d'annuels (sur les 26\,115) $+$ \textbf{7\,478} de scans.
