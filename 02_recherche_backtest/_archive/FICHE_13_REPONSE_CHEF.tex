...
\hrule height 0.6pt
\vspace{1mm}
{\footnotesize\color{grisf}\textbf{État au 22/07/2026 --- document figé.} Il porte sur la ligne
\textbf{par membre}, close depuis (verdict : pas d'$\alpha$). La ligne \textbf{par titre}, qui est le livrable
courant, n'a repris que les corrections (i) et (ii) de R3 --- régression en excès de $r_f$ et modèle à quatre
facteurs ; les sept autres remarques restent \textbf{sans réponse écrite ailleurs}, c'est la raison d'être de
ce document. Les neuf opérateurs proposés ici sont un \textbf{pré-enregistrement} : aucun run primaire n'a été
exécuté, et le test de validité déclaré d'avance (\og{}Fetterman et Ruiz doivent tomber\fg{}) n'a jamais été
lancé. \textbf{Compteur d'essais de la ligne titre} : 84 runs, \texttt{FICHE\_M3} annexe D --- les
\textbf{160 essais} de l'encadré \emph{Le budget d'essais} comptent la ligne membre et ne s'y ajoutent pas.\par}
\vspace{1mm}
\hrule height 0.6pt
...
Ce compteur est celui de la \textbf{ligne membre} : il ne s'additionne pas aux \textbf{84 runs} de la ligne
titre (\texttt{FICHE\_M3} annexe D), qui compte un autre périmètre.
