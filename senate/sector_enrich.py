...
# --- Ancrage : le module vit dans <repo>/senate/ ; données sous data/senate (parité House) ---
BASE_DIR = Path(__file__).resolve().parent
OUT = BASE_DIR.parent / "data" / "senate"
TABLE_DIR = OUT
SECTOR_CACHE = OUT / "sector_cache.json"
...
def resolve_sectors(tickers, with_llm: bool = True, audit_all: bool = True) -> dict:
    ...
    if todo:
        ...
        save_cache(cache)
        print(f"  cache sauvegardé : {SECTOR_CACHE.name} ({len(cache)} tickers)")
