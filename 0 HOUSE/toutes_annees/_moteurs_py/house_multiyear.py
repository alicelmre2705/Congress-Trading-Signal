...
- Réutilise les PDF + index déjà téléchargés dans `semaine 1/` (aucun re-téléchargement).
- OCR DIFFÉRÉ : les PDF scannés (sans texte) ne sont PAS traités ici, seulement inventoriés
  dans data_v1/tables/00_backlog_ocr.csv (+ BACKLOG_OCR.md) pour traitement ultérieur via la
  méthode PDF/Vision de Claude (notebook_v1_house_2025q1_ocr.ipynb).
...
# ───────────────────────── Chemins & constantes ─────────────────────────
# Robuste au rangement du script : on trouve le dossier "0 HOUSE" (celui qui contient data_v1)
# et la racine projet "Jupiter" (celle qui contient "semaine 1"), peu importe la profondeur.
HERE = Path(__file__).resolve().parent
HOUSE_DIR = next((p for p in [HERE, *HERE.parents] if (p / "data_v1").is_dir()), HERE)
PROJECT = next((p for p in [HOUSE_DIR, *HOUSE_DIR.parents] if (p / "semaine 1").is_dir()), HOUSE_DIR.parent)
load_dotenv(PROJECT / ".env")

SEM1_PDF   = PROJECT / "semaine 1" / "data" / "raw" / "house" / "ptr_pdfs"
SEM1_INDEX = PROJECT / "semaine 1" / "data" / "raw" / "house" / "index"
SEM1_PARSE = PROJECT / "semaine 1" / "data" / "processed" / "house" / "house_transactions.csv"
SEM1_REF   = PROJECT / "semaine 1" / "data" / "reference" / "raw"

OUTDIR  = HOUSE_DIR / "data_v1"
...
def _load_people():
    """current + historical. Live d'abord ; repli sur les YAML locaux de semaine 1."""
    try:
...
def load_ptr_index(year, win_start, win_end):
    """Lit l'index XML de semaine 1, filtre FilingType=P + fenêtre disclosure."""
...
def build_manifest(year, ptr_index, ydir):
    """Lit les PDF de semaine 1 EN PLACE, extrait le texte, route lisible/non_lisible.
...
# ───────────────────────── Cross-check vs baseline semaine 1 ─────────────────────────
_SEM1_CACHE = {}
def _sem1_year(year):
...
def crosscheck_semaine1(df, ptr_index, year, ydir):
    ...
    total = len(cmp)
    # semaine 1 a un parser plus faible : on attend nous >= sem1. Le vrai signal de dérive de
    # format = un doc parsé par semaine 1 mais où NOUS sortons 0 ligne.
