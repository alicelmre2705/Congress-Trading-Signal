...
MANUAL_OVERRIDES = {
    ...
    "LRN": "Consumer Discretionary",    # Stride/éducation (yfinance disait Consumer Staples à tort)
    # — ETF thématiques/sectoriels : un secteur dominant clair —
    "SHLD": "Industrials",              # ETF défense (ticker réattribué, ex-Sears)
    "HURA": "Materials",                # ETF uranium miners (était Health Care)
    "URNM": "Materials",                # ETF uranium miners — Metals & Mining, pas Energy
    # — ETF broad / diversifiés / commodités / crypto : PAS de secteur unique -> None —
    "SLYG": None,                       # S&P 600 small-cap growth (diversifié)
    "TNA": None,                        # Direxion small-cap 3x (diversifié)
    "SPY": None,                        # S&P 500 (les 11 secteurs)
    "QQQ": None,                        # Nasdaq-100 (multi-secteurs)
    "IVW": None,                        # S&P 500 Growth (diversifié)
    "PDBC": None,                       # commodités diversifiées
    "BITB": None,                       # bitcoin spot (aucun titre actions)
}
...
# ======================================================================
# CLI standalone (toutes_annees) : enrichit les 7 tables digitales annuelles.
# Stratégie : un SEUL run réseau sur l'UNION des tickers (remplit le cache),
# puis enrichissement de chaque table (cache hits). Cache bootstrappé depuis
# le pilote House 2025_test (même pipeline_sha) -> ne ré-interroge que les
# tickers nouveaux (années 2020-2024).
# ======================================================================
import glob as _glob


def _order_cols(cols):
    """sector_gics/etf_proxy juste après asset_type ; sector_source en fin (ordre stable)."""
    base = [c for c in cols if c not in ("sector_gics", "etf_proxy", "sector_source")]
    out = []
    for c in base:
        out.append(c)
        if c == "asset_type":
            out += ["sector_gics", "etf_proxy"]
    if "sector_gics" not in out:        # filet si pas d'asset_type
        out += ["sector_gics", "etf_proxy"]
...
def main() -> None:
    paths = sorted(_glob.glob(str(TABLE_DIR / "*" / "06_house_*_transactions.csv")))
    print(f"{len(paths)} tables annuelles digitales")
    dfs, union = {}, set()
    for p in paths:
        ...
        for t in df["ticker"].dropna():
            t = _norm_ticker(t)
            if t and t not in ("NAN", "NONE", "") and _VALID_TICKER.match(t):
                union.add(t)
    print(f"Union tickers distincts (7 années) : {len(union)}")

    resolve_sectors(sorted(union))   # un seul run réseau, remplit/complète le cache

    tot = cov = 0
    for p, df in dfs.items():
        df2 = enrich_sectors(df)     # cache hits -> instantané
        df2 = df2.reindex(columns=_order_cols(df2.columns))
        df2.to_csv(p, index=False)
        n, c = len(df2), int(df2["sector_gics"].notna().sum())
        tot += n
        cov += c
        from pathlib import Path as _P
        print(f"  {_P(p).parent.name} : {c}/{n} lignes classées")

    big = pd.concat(list(dfs.values()), ignore_index=True)
    audit = build_audit(big)
    audit.to_csv(TABLE_DIR / "06f_sector_audit.csv", index=False)
    print(f"\n→ 06f_sector_audit.csv : {len(audit)} tickers distincts")
    summary(audit)
    print(f"\nCouverture sector_gics (toutes années) : {cov}/{tot} = {cov / tot:.1%}")
