...
| **Couverture secteur** (`sector_gics`) | **43,7 %** = 94 % des actifs *tickérisables* (cf. §4) |
| **Champs obligatoires** | **12/12** (`sector_gics` + `etf_proxy` ajoutés) |
...
## 4. Secteur GICS → ETF (`sector_gics`, `etf_proxy`)

Les deux derniers champs obligatoires. Même module et méthode que le pilote House Q1
(`sector_enrich.py`, partagé) : secteur **factuel via yfinance** (primaire), **repli LLM** (Claude
`tool_use`, cache versionné) pour delistés/préférentielles, **couche d'override** d'audit. Univers
ETF cible = **SPDR Select Sector** (1:1 avec les 11 secteurs GICS). Audit : `06f_sector_audit.csv`.

### Couverture (107 tickers distincts → 375 lignes)

| Indicateur | Valeur |
|---|---|
| `sector_gics` / `etf_proxy` remplis | 164/375 lignes = **43,7 %** |
| — en part des actifs *tickérisables* (191 lignes) | **94 %** |
| Source `yfinance` (factuel) | 151 lignes |
| Source `llm` (repli) | 9 lignes |
| Source `manual` (override audit) | 20 lignes / 7 tickers |
| `none` (munis, obligations, OCR privés, ETF broad) | 211 lignes — `null` à dessein |

La couverture (43,7 %) suit le **plafond ticker** : 184 lignes sont des munis/obligations/véhicules
privés OCR sans ticker boursier. Chaque `sector_gics` non-null a un `etf_proxy` ∈ 11 SPDR (jointure
1:1 complète, 0 manquant).

### Validation

- **Audit croisé yfinance ↔ LLM** : sur 82 tickers résolus par les deux, **98,8 % d'accord** (1 seul
  désaccord : LRN, Stride/éducation — le LLM avait raison, corrigé en Consumer Discretionary).
- **Audit adversarial** (juge indépendant sur la queue mono-source) : **7 corrections** appliquées —
  5 **ETF de marché diversifiés** (SPY, QQQ, IVW, PDBC, BITB) classés à tort par secteur → **non
  classés** (un indice/commodité/crypto n'a pas de secteur GICS unique) ; URNM (uranium *miners*) →
  **Materials** (Metals & Mining, pas Energy) ; LRN → **Consumer Discretionary**. Verdict : mapping
  **fiable**.

> Cohérence inter-périmètres : ces overlays (ETF broad → non classés, etc.) sont centralisés dans le
> `MANUAL_OVERRIDES` partagé par les modules House et Sénat — corrections factuelles universelles.

---

## 5. Limites connues (où les stats sont bonnes / pas)
...
- **Secteur — multi-sources & ETF broad.** `sector_gics` = yfinance (factuel) + repli LLM + 7
  overrides d'audit ; pas de vérité-terrain unique (confiance via l'audit croisé 98,8 %). Les ETF de
  marché diversifiés (SPY, QQQ…) sont laissés non classés à dessein. Univers `etf_proxy` = SPDR
  Select Sector, à réaligner sur l'univers Ramify en fin de projet.
