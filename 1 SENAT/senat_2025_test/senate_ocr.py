#!/usr/bin/env python
"""OCR autonome — PTR Sénat SCANNÉS (papier), Q1 2025.

Dossier 100 % autonome : AUCUNE dépendance vers « 0 HOUSE/ » ni « semaine 1/ ».
On lit les rapports papier déjà cachés (data_v1_senate/reports/{uuid}.html), on récupère
les images .gif (efd-media-public.senate.gov — serveur public, hors barrière CSRF), on les
cache en local, puis on les passe à Claude Vision (tool_use `record_transactions`).
Cache OCR versionné par prompt_sha + modèle (re-run = 0 appel si inchangé).
...
Sortie : data_v1_senate/tables/06b_senate_2025q1_ocr_transactions.csv (provenance=senate-efd-ocr),
au schéma complet (identité ré-attachée via senate_finalize). La fusion digital+OCR et la
re-validation Quiver sont faites par senate_finalize.py.
"""
import os
import re
import io
...
from senate_finalize import (load_reference, make_matcher, recover_ticker,
