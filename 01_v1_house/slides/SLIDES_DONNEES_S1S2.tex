...
\begin{frame}{Les autres dépôts (1/13) — le cycle de vie d'un élu}
  \centering
...
\begin{frame}{Les autres dépôts (2/13) — le même formulaire, des sections en commun}
  \centering
...
% ── AD · À quoi servent les sections A→J, pourquoi on ne garde que B ──
\begin{frame}{Les autres dépôts (3/13) — les sections A→J : laquelle est un signal ?}
  \centering
  \includegraphics[width=0.90\textwidth]{slides/fig_sections_roles.png}\\[0.8mm]
  {\scriptsize\color{black!60} on trade \textbf{B} (transactions datées) ;
   \textbf{A} sert seulement à \emph{vérifier} qu'on n'a rien loupé}
\end{frame}

% ── AD1 · La famille patrimoine ── (vignettes cliquables ; profil mesuré sur 5 815 docs)
\begin{frame}{Les autres dépôts (4/13) — la famille « patrimoine »}
  \centering
...
\begin{frame}{Les autres dépôts (5/13) — \texttt{O}, le rapport annuel}
  \begin{columns}[c]
...
\begin{frame}{Les autres dépôts (6/13) — \texttt{C}, le rapport de candidat}
  \begin{columns}[c]
    \begin{column}{0.34\textwidth}\centering
...
\begin{frame}{Les autres dépôts (7/13) — \texttt{H}, la déclaration d'entrée}
  \begin{columns}[c]
    \begin{column}{0.34\textwidth}\centering
...
\begin{frame}{Les autres dépôts (8/13) — \texttt{T}, la déclaration de sortie}
  \begin{columns}[c]
    \begin{column}{0.34\textwidth}\centering
...
\begin{frame}{Les autres dépôts (9/13) — \texttt{A}, l'amendement}
  \begin{columns}[c]
    \begin{column}{0.34\textwidth}\centering
...
\begin{frame}{Les autres dépôts (10/13) — surprise : il y a des transactions dedans}
  \centering
  ...
  {\footnotesize la \textbf{Schedule B} d'un rapport annuel \textbf{récapitule les
   ...
   delta 0 jour) — mais est-ce le cas de tout le monde ? → slide 12/13 \quad
...
\begin{frame}{Les autres dépôts (11/13) — les administratifs, et le cas à part}
  \centering
...
\begin{frame}{Les autres dépôts (12/13) — que rate-t-on ? (mesuré)}
  \begin{columns}[c]
...
\begin{frame}{Les autres dépôts (13/13) — qu'est-ce qu'on pourrait faire ?}
  \centering
  \begin{columns}[T]
...
% ═════ Validation Chambre : entrée (H) + transactions (P) = sortie (T) ? ═════
% chiffres : docs/ANALYSE_COHERENCE_PORTEFEUILLE.md (PoC, 33 élus H+T)

% ── VC1 · l'idée du contrôle ──
\begin{frame}{La Chambre — valider qu'on a tout : entrée + transactions = sortie ?}
  \centering
  {\footnotesize on a montré (aparté) que le rapport d'\textbf{entrée (H)} et de
   \textbf{sortie (T)} listent le \textbf{patrimoine détenu} (Schedule A). D'où un test
   de complétude :}\\[4mm]
  \resizebox{0.97\textwidth}{!}{%
  \begin{tikzpicture}[node distance=14mm]
    \node[step=okgreen, text width=3.5cm, minimum height=1.7cm, font=\footnotesize] (h)
      {\textbf{patrimoine d'ENTRÉE}\\rapport \co{H}\\{\scriptsize ce qu'il détient en arrivant}};
    \node[step=helec, text width=3.5cm, minimum height=1.7cm, font=\footnotesize, right=of h] (p)
      {\textbf{+ toutes les transactions}\\les \co{PTR} du mandat\\{\scriptsize achats / ventes datés}};
    \node[step=dataC, text width=3.5cm, minimum height=1.7cm, font=\footnotesize, right=of p] (t)
    ...
    \node[below=6mm of p, font=\small\bfseries, color=alertred] {on doit retomber dessus — sinon, il manque une transaction};
  \end{tikzpicture}}
  \\[5mm]
  {\footnotesize testé sur les \textbf{33 élus} entrés \textbf{et} sortis entre 2014 et 2026
   (ceux qui ont un H \emph{et} un T) — reconstruction titre par titre}
\end{frame}

% ── VC2 · le résultat ──
\begin{frame}{La Chambre — résultat : 80 \% du portefeuille de sortie reconstruit}
  \centering
  \includegraphics[width=0.9\textwidth]{coherence/fig_coherence.png}\\[2mm]
  \begin{columns}[T]
    \begin{column}{0.63\textwidth}
      {\footnotesize
      · \badge{okgreen}{80 \% cohérent} — entrée + nos transactions reconstruisent le
        portefeuille de sortie → \textbf{flux PTR largement complet}\\[1.2mm]
      · \textbf{27 \% détenus tout du long, jamais tradés} → \textbf{invisibles} à la
        reconstruction actuelle (qui part de zéro) : le \textbf{patrimoine d'entrée est
        la brique qui manque}}
    \end{column}
    \begin{column}{0.35\textwidth}
      {\scriptsize\color{black!70}
      \textbf{exemple} — M.\ T.\ Greene détient
      \href{https://disclosures-clerk.house.gov/public_disc/financial-pdfs/2020/10042733.pdf}{BMY \& MCD à l'entrée}
      et à la
      \href{https://disclosures-clerk.house.gov/public_disc/financial-pdfs/2026/10073322.pdf}{sortie}, \textbf{0 PTR}.\\[1.2mm]
      9 élus sur 33 n'ont \textbf{presque aucun PTR} mais détiennent des actions →
      le patrimoine est leur \textbf{seule} fenêtre.}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[c]
    \begin{column}{0.42\textwidth}\centering
      \href{https://efd-media-public.senate.gov/media/2015/3/000/002/000002618.gif}{%
        \fcolorbox{black!30}{white}{\includegraphics[height=47mm]{slides/senat_papier_couche.png}}}\\[1mm]
    ...
    \begin{column}{0.42\textwidth}\centering
      \href{https://efd-media-public.senate.gov/media/2020/2/000/000/000000936.gif}{%
        \fcolorbox{black!30}{white}{\includegraphics[height=47mm]{slides/senat_papier_droit.png}}}\\[1mm]
  ...
  \begin{columns}[c]
    \begin{column}{0.42\textwidth}\centering
      \href{https://efd-media-public.senate.gov/media/2014/1/000/012/000012036.gif}{%
        \fcolorbox{black!30}{white}{\includegraphics[height=48mm]{slides/senat_papier_manuscrit.png}}}\\[1mm]
    ...
    \begin{column}{0.42\textwidth}\centering
      \href{https://efd-media-public.senate.gov/media/2018/2/000/001/000001730.gif}{%
        \fcolorbox{black!30}{white}{\includegraphics[height=48mm]{slides/senat_papier_annexe.png}}}\\[1mm]
...
% ═════════════ V. LA REFONTE V1 — chiffres : cache/MANIFEST.json (run 2026-07-13) ═════════════
\section{La refonte V1 — la Chambre refaite et prouvée}

% ── V1-0 · Pourquoi et quoi ── (tuiles = MANIFEST.json)
\begin{frame}{La V1 en un coup d'œil — un notebook qui se prouve lui-même}
  \centering
  {\footnotesize toute la chaîne Chambre refaite dans \textbf{un seul notebook autonome}
   (\co{01\_v1\_house/V1\_House.ipynb}) — et \textbf{validée de l'intérieur}, sans source externe}\\[3mm]
  \tuile{helec}{35 377}{dépôts recensés\\(index site $\cup$ instantané)}\hfill
  \tuile{helec}{23 245}{PDF en cache\\(2 Go, sha256, 1 seul 404)}\hfill
  \tuile{okgreen}{56 002}{transactions signal\\(PTR électroniques)}\hfill
  \tuile{okgreen}{198 615}{holdings Schedule A\\(patrimoine, 1\iere{} fois)}\\[2mm]
  \tuile{dataC}{110 147}{transactions contexte\\(annuels O/T/A/C)}\hfill
  \tuile{dataC}{6+1}{types parsés\\(P + O/H/T/A/C)}\hfill
  \tuile{okgreen}{37}{validations\\\textbf{0 rouge}}\hfill
  \tuile{grisC}{100 \%}{couverture réelle\\de l'électronique}\\[3mm]
  {\scriptsize\color{black!60} règle d'or héritée de l'incident « 50/50 vs 71/29 » :
   \textbf{aucun ratio sans ses trois étiquettes} — unité × périmètre × fenêtre}
\end{frame}

% ── V1-1 · La source s'efface ── (fig recalculée depuis cache/index*)
\begin{frame}{Découverte — la source primaire s'efface sous nos yeux}
  \centering
  \includegraphics[width=0.94\textwidth]{fig_v1_purge_index.png}\\[1mm]
  {\footnotesize le Clerk \textbf{purge} les index au-delà de $\sim$6 ans (STOCK Act) :
   les \textbf{PDF répondent encore}, mais la \emph{liste de quoi demander} a disparu du site}\\[0.8mm]
  {\scriptsize\color{black!60} → notre \textbf{instantané} est un actif critique ;
   le recensement V1 = union \{site aujourd'hui\} $\cup$ \{instantané\}, chaque ligne tracée —
   et au passage \textbf{+15 nouveaux PTR} de juillet captés}
\end{frame}

% ── V1-2 · Le ledger ── (fig depuis 02_ledger_docs.csv)
\begin{frame}{Le ledger — plus aucun document ne disparaît sans trace}
  \centering
  \includegraphics[width=0.97\textwidth]{fig_v1_ledger.png}\\[1.5mm]
  {\footnotesize l'ancienne métrique de couverture valait 100 \%
   \textbf{par construction} (une partition déguisée) ; celle-ci peut échouer —
   c'est ce qui lui donne sa valeur}
\end{frame}

% ── V1-3 · Les témoins ── (asserts littéraux du notebook, §7.2/§8.2/§8.3)
...
% ── V1-4 · Le procès de l'ancien ── (X1/X2, arbitrage marqueurs)
...
% ── V1-5 · Cohérence portefeuille ── (C-a/C-b + §11.7 blind trusts)
\begin{frame}{L'idée maîtresse validée : les portefeuilles se recoupent}
  \centering
  \includegraphics[width=0.99\textwidth]{fig_v1_coherence.png}\\[1.5mm]
  {\footnotesize et l'hypothèse « blind trust » vérifiée (§11.7) : le portefeuille visible
   \textbf{s'effondre} à l'entrée du trust (Phillips : 284 $\to$ 1 position, repliées dans
   une ligne opaque « Qualified Blind Trust » \$5-25M) — une \emph{disparition}, pas un écart ;
   \textbf{0} position inexpliquée de la cohorte ne vient d'un trust}
\end{frame}

% ── V1-6 · Tableau de bord + suites ──
\begin{frame}{Le tableau de bord — et la suite}
  \centering
  \includegraphics[width=0.88\textwidth]{fig_v1_dashboard.png}\\[1.5mm]
  {\footnotesize\badge{helec}{V2 : OCR des 5 276 scannés}\;
   \badge{selec}{V-Sénat sur le même modèle}\;
   \badge{okgreen}{S3 : partir du patrimoine d'entrée (H)}}\\[1mm]
  {\scriptsize\color{black!60} les 4 orange sont des \emph{mesures documentées}
   (PTR papier hors V1, codes de district exotiques) — le notebook les liste une à une}
\end{frame}
...
% ── S3-1 · Pont : le pipeline de la question ──
\begin{frame}{La donnée est prête — la question change : peut-on la trader ?}
  \centering
  {\footnotesize 118\,029 trades datés, chiffrés, tracés — reste LA question :
   \textbf{copier les élus rapporte-t-il de l'alpha ?}}\\[4mm]
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[node distance=5mm]
    \node[step=dataC, text width=2.9cm] (d) {\co{transactions\_ backtest\_2014\_2026}\\[0.5mm]134\,429 trades · 372 élus};
    \node[step=helec, text width=2.6cm, right=of d] (a) {\textbf{A. le moteur}\\[0.5mm]1 élu = 1 portefeuille en parts : $n_i(t)$, NAV};
    \node[step=helec, text width=2.6cm, right=of a] (b) {\textbf{B. la mesure}\\[0.5mm]qui bat le SPY ?\\IR vs appraisal};
    \node[step=helec, text width=2.6cm, right=of b] (c) {\textbf{C. la stratégie}\\[0.5mm]copier le top-$K$\\walk-forward};
    \node[step=grisC, text width=2.6cm, right=of c] (v) {\textbf{D. les variantes}\\[0.5mm]$K$ · cadence · poids $w$ · $\Sigma$};
    \node[step=selec, text width=2.6cm, right=of v] (s) {\textbf{E. rejouer au Sharpe}\\[0.5mm](Partie II)};
    \node[step=alertred, text width=1.9cm, right=of s] (f) {\textbf{le verdict}\\[0.5mm]$\alpha \overset{?}{>} 0$};
    \draw[arr] (d) -- (a); \draw[arr] (a) -- (b); \draw[arr] (b) -- (c);
    \draw[arr] (c) -- (v); \draw[arr] (v) -- (s); \draw[arr] (s) -- (f);
  \end{tikzpicture}}\\[4mm]
  \badge{helec}{Partie I : significativité (§1-§10)}\;
  \badge{selec}{Partie II : Sharpe (§11-§12)}\;
  \badge{grisC}{notebook 05b — chaque chiffre recalculé de zéro}\\[2mm]
  {\scriptsize\color{black!60} une seule question, posée de six façons différentes —
   et un résultat \textbf{négatif}, assumé et prouvé}
\end{frame}

% ── S3-2 · §0 Données & prix ──
\begin{frame}{D'abord blinder les prix : 118\,029 trades backtestables sur 134\,429}
  \centering
  \tuile{dataC}{134 429}{trades déclarés\\(dates 2013-2026)}\hfill
  \tuile{dataC}{372}{membres\\du Congrès}\hfill
  \tuile{dataC}{4 618}{tickers\\distincts}\hfill
  \tuile{grisC}{SPY}{benchmark\\= calendrier maître}\\[2mm]
  \tuile{alertred}{1 268 + 61}{tickers sans prix\\ou corrompus}\hfill
  \tuile{alertred}{16 400}{trades jetés\\avec eux}\hfill
  \tuile{okgreen}{118 029}{trades\\exploitables}\hfill
  \tuile{okgreen}{266}{portefeuilles\\reconstructibles}\\[3mm]
  \badge{alertred}{prix < 0,10 \$}\;
  \badge{alertred}{saut > 300 \%/jour}\;
  \badge{alertred}{< 2 points de prix}\quad
  {\footnotesize → tout prix douteux disqualifie le \textbf{ticker entier}}\\[1.5mm]
  {\scriptsize\color{black!60} prix Yahoo en cache figé (\co{prices\_v2}, ffill sur le calendrier SPY) ;
   les 743 trades exécutés en 2013 (déclarés en 2014+) sont gardés}
\end{frame}

% ── S3-3 · §1 Le moteur : reconstruction en parts ──
\begin{frame}{Le moteur — un flux de trades devient un portefeuille en parts}
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      {\footnotesize $P_i(t)$ = cours de clôture du ticker $i$ ;\\
       montant = \textbf{milieu de la fourchette} STOCK Act ;\\
       $\tau$ = 1\iere{} séance cotée $\geq$ date du trade}\\[2mm]
    ...
    \begin{column}{0.51\textwidth}\centering
      ...
      {\scriptsize\color{black!60} la valeur $V(t)$ mélange apports et performance
       → il faut un rendement qui les sépare}
    \end{column}
  \end{columns}
\end{frame}

% ── S3-4 · §2 Rendement time-weighted ──
\begin{frame}{Le rendement time-weighted : un apport n'est jamais une performance}
  \centering
  {\footnotesize on valorise \textbf{les parts d'hier} aux prix d'aujourd'hui — l'argent frais n'entre qu'au lendemain}\\[1mm]
  $r_P(t)=\dfrac{\sum_i n_i(t{-}1)\,P_i(t)}{\sum_i n_i(t{-}1)\,P_i(t{-}1)}-1
   \qquad\quad \mathrm{NAV}(t)=\prod_{s\le t}\bigl(1+r_P(s)\bigr)$\\[1.5mm]
  \badge{warnorange}{winsorisation ±50 \%/jour}\;
  \badgel{hocr}{jour sans position : $r_P=0$}\\[2mm]
  \includegraphics[height=0.5\textheight]{fig_05b_spaghetti_nav.png}
\end{frame}
...
% ── S3-6 · §4 Les deux mètres : IR vs appraisal ──
\begin{frame}{Deux mètres pour « battre le SPY » : l'écart brut (IR) ou l'alpha ajusté du $\beta$}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      {\scriptsize même échelle de test : $t=\text{critère}\times\sqrt{n/252}$,
       unilatéral, seuil $t>1{,}645$}
    \end{column}
    \begin{column}{0.44\textwidth}\centering
...
% ── S3-7 · §4.1-4.2 Résultat : 20 → 12 ≈ hasard ──
\begin{frame}{20 « gagnants » au mètre IR… 12 au mètre appraisal — le hasard en promettait 11}
  \centering
  \includegraphics[height=0.52\textheight]{fig_05b_hist_t.png}\\[1.5mm]
  \badge{helec}{IR : 20 membres à $t>1{,}645$}\;
  \badge{selec}{appraisal : 12}\;
  \badge{grisC}{attendus sous H$_0$ : ≈ 11 (5 \% de 223)}\\[1.5mm]
  {\scriptsize\color{black!60} ajusté du $\beta$, le surnombre de « talents » retombe au niveau du bruit —
   la tête (Fetterman, IR 2,48) tient sur 284 jours seulement}
\end{frame}

% ── S3-8 · §4.3 Le lien IR ↔ appraisal ──
\begin{frame}{La contre-preuve point par point : la queue droite de l'IR, c'était du $\beta$}
  ...
  {\scriptsize\color{black!60} on garde les DEUX mètres pour la suite :
   chaque stratégie sera jugée en excès brut \textbf{et} en $t_{\mathrm{appr}}$}
\end{frame}

% ── S3-9 · §5 Walk-forward point-in-time ──
\begin{frame}{Ne jamais regarder le futur : classé fin $Y$, copié pendant $Y{+}1$}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}\centering
      \vspace{2mm}
      \resizebox{\linewidth}{!}{%
      \begin{tikzpicture}
        \draw[arr] (-0.4,0) -- (7.6,0);
        \foreach \x in {0.6, 3.4, 6.2} \draw[line width=1pt, gray] (\x,-0.12) -- (\x,0.12);
        \draw[line width=2.4pt, okgreen] (3.4,0) -- (6.2,0);
        \node[below, font=\scriptsize, gray] at (0.6,-0.15) {31/12/$Y{-}1$};
        \node[below, font=\scriptsize] at (3.4,-0.15) {\textbf{coupe 31/12/$Y$}};
        \node[below, font=\scriptsize, okgreen] at (6.2,-0.15) {31/12/$Y{+}1$};
        \node[below, font=\scriptsize\bfseries, okgreen] at (4.8,-0.62) {année de tenue $Y{+}1$};
        \node[step=helec, text width=4.5cm, above=9mm of {(2.4,0)}] (cl)
          {$\mathrm{classement}(Y)$ : éligibles \textbf{et} $t_{\text{crit}}>1{,}645$,\\
           triés par le critère (IR ou appraisal)};
        \node[step=okgreen, text width=2.6cm, above=9mm of {(6.0,0)}] (tk)
          {\textbf{top-$K$} suivi toute l'année $Y{+}1$};
        \draw[arr] (cl.south) -- (3.4,0.18);
        \draw[arr] (cl.east) -- (tk.west);
      \end{tikzpicture}}\\[3mm]
      \badge{alertred}{aucune info postérieure à la coupe}\\[1.5mm]
      {\scriptsize\color{black!60} 11 coupes : fin 2015 → fin 2025 ;\\
       s'il y a moins de $K$ significatifs, on copie tous les significatifs}
    \end{column}
    \begin{column}{0.46\textwidth}\centering
      \includegraphics[width=\linewidth]{fig_05b_nsig_par_an.png}\\[1mm]
      {\scriptsize\color{black!60} le vivier n'est jamais vide —
       de quoi alimenter un top-$K$ chaque année (au pire 2 noms, IR fin 2016)}
    \end{column}
  \end{columns}
\end{frame}

% ── S3-10 · §6 Le moteur en parts (la NAV du chef) ──
\begin{frame}{Copier en parts : chaque élu devient un fonds — compter, redéployer, tenir}
  \centering
...
% ── S3-11 · §6.3 Résultat top-4 ──
\begin{frame}{Top-4 : +3,1 \%/an d'excès… $t = 0{,}94$ — et l'alpha dit zéro}
  \centering
  ...
  \includegraphics[height=0.47\textheight]{fig_05b_nav_top4.png}\\[1.5mm]
  \tuile{helec}{+3,1 \%/an}{excès top-4 IR\\7/11 années gagnantes}\hfill
  \tuile{alertred}{t = 0,94}{< 1,81 — indiscernable\\du hasard}\hfill
  \tuile{warnorange}{$\beta$ = 1,20}{l'excès = du levier\\de style, $t_{\text{appr}}$ −0,13}\hfill
  \tuile{grisC}{−2,5 \%/an}{le mètre « propre »\\(appraisal) fait pire}
\end{frame}

% ── S3-12 · §7-§8 K et cadence ──
\begin{frame}{Optimiser $K$, accélérer la cadence : le $t$ ne franchit jamais le seuil}
  \centering
  {\footnotesize $K^{*}(Y)=\arg\max_{K\in[4,20]}\ \mathrm{score}\big(K \,\big|\, \text{passé} \le t^R_Y\big)$
   — le choix de $K$ est lui-même walk-forward ;\quad
   6 mois : coupes 30/06 \& 31/12, $n=22$, seuil $1{,}72$}\\[1mm]
  \includegraphics[width=0.67\textwidth]{fig_05b_k_et_cadence.png}\\[1.2mm]
  \badge{helec}{K walk-forward : +4,1 \%/an, t = 1,58 < 1,81}\;
  \badge{warnorange}{6 mois : +0,4 \%/an, t = 0,14}\;
  \badge{grisC}{le K choisi saute : 4 → 8 → 18 → 12…}\\[1.5mm]
  {\scriptsize\color{black!60} plus de degrés de liberté = plus de chances de sur-ajuster — pas plus d'alpha}
\end{frame}

% ── S3-13 · §9 Pondérations ──
\begin{frame}{Quatre façons de pondérer, zéro alpha : max $t_{\text{appr}} = +0{,}48$}
  \begin{columns}[T]
    \begin{column}{0.44\textwidth}\centering
      \begin{tikzpicture}[node distance=3.2mm]
        \node[step=grisC, text width=5.3cm] (w1) {$1/K$ — ignore tout};
        \node[step=helec, text width=5.3cm, below=of w1] (w2)
          {inverse-vol : $w_k=\dfrac{1/\sigma_k}{\sum_j 1/\sigma_j}$ — lit les variances};
        \node[step=helec, text width=5.3cm, below=of w2] (w3)
          {ERC : $w_k(\Sigma w)_k = w_j(\Sigma w)_j\ \forall k,j$\\[0.5mm]
           GMV : $\min_{w\ge 0,\ \mathbf{1}^{\top}w=1}\ w^{\top}\Sigma\,w$ — lisent $\Sigma$ entière};
        \node[step=grisC, text width=5.3cm, below=of w3] (w4)
          {Markowitz complet — \textbf{refusé} :\\il faudrait prédire les rendements $\mu$};
        \draw[arr] (w1) -- (w2) node[midway, right=1mm, font=\tiny, gray] {+ d'info};
        \draw[arr] (w2) -- (w3) node[midway, right=1mm, font=\tiny, gray] {+ d'info};
        \draw[arr] (w3) -- (w4);
      \end{tikzpicture}\\[1.5mm]
      {\scriptsize\color{black!60} $\sigma_k$, $\Sigma$ estimés \textbf{point-in-time}\\
       (rendements $\le t^R_Y$, fenêtre commune)}
    \end{column}
    \begin{column}{0.54\textwidth}\centering
      \includegraphics[width=0.96\linewidth]{fig_05b_ponderations.png}\\[1mm]
      \badgel{hocr}{inverse-vol : t 0,94 → 1,49 — sans $\alpha$}\;
      \badge{alertred}{GMV sél. appr : −4,5 \%/an, t −2,76}
    \end{column}
  \end{columns}
\end{frame}

% ── S3-14 · §10 Ledoit-Wolf ──
...
% ── S3-15 · §11 Partie II : le Sharpe ──
\begin{frame}{Partie II — on change la règle du jeu : le Sharpe brut, sans test}
  \centering
  {\footnotesize $\mathrm{Sharpe}=\dfrac{\overline{r_P}}{\sigma(r_P)}\sqrt{252}$ ;\quad
   éligible v2 $\iff n_{\text{jours}}\ge 126$ \textbf{et} $\mathrm{Sharpe}>0{,}4$
   — plus de minimum de trades, \textbf{plus aucun test t}}\\[1.5mm]
  \includegraphics[height=0.5\textheight]{fig_05b_hist_sharpe.png}\\[2mm]
  \badge{selec}{224 éligibles v2}\;
  \badgel{socr}{Partie I : 223 — presque les mêmes visages}\;
  \badge{warnorange}{les têtes = historiques courts : Fetterman 284 j, Sewell 4 trades}\\[1.5mm]
  {\scriptsize\color{black!60} on classe au rendement/risque brut —
   au prix d'admettre des historiques courts en tête de classement}
\end{frame}

% ── S3-16 · §12 Résultat Partie II ──
\begin{frame}{5 pondérations × $K$ de 2 à 20 × 2 cadences : le max plafonne à $t = 1{,}22$}
  \centering
  \includegraphics[width=0.57\textwidth]{fig_05b_balayage_k.png}\\[1mm]
  \badge{selec}{meilleure table : K=5, 1/K, annuel — +3,5 \%/an, t = 1,10}\;
  \badge{selec}{max du balayage : K=6 — t = 1,22, t\_appr = 0,99}\\[0.8mm]
  \badgel{socr}{Sharpe 0,93 vs SPY 0,87 — le couple rendement/risque ne bouge pas}\;
  \badge{alertred}{dilution : +3,5 \% (K=5) → +0,7 \% (K=10) → −0,2 \% (K=20) ; GMV : −6,5 \%/an}\\[1mm]
  {\scriptsize\color{black!60} le signal, s'il existe, ne survit ni à la largeur du panier ni au mètre ajusté —
   pas mieux que la Partie I}
\end{frame}
...
\begin{frame}{Verdict : un $\beta$ de style $\approx$ 1,2 — et zéro alpha, sous toutes les coutures}
  ...
  \end{tikzpicture}}\\[4mm]
  \tuile{alertred}{1,58}{max $t$, Partie I\\(seuil 1,81)}\hfill
  \tuile{alertred}{1,22}{max $t$, Partie II\\(seuil 1,81)}\hfill
