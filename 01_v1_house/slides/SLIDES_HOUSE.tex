...
\begin{frame}{Comment on entre au Congrès : qui dépose quoi, et quand (la loi)}
  \centering
  \includegraphics[width=0.99\textwidth]{fig_house_loi_cycle.png}\\[1mm]
  {\footnotesize \badge{alertred}{2 règles non écrites}\;
   un candidat déjà déclaré \textbf{saute le H} ; qui part au Sénat / à l'exécutif / décède
   \textbf{ne dépose pas de T}}\\[0.6mm]
  {\scriptsize\color{black!55} rien de tout cela n'est documenté ailleurs —
   reconstruit des sources primaires (loi + guide du Comité d'éthique)}
\end{frame}
...
% ── H16 · Le tableau de bord ── (MANIFEST.json : 48 validations, 0 rouge ; fig_house_dashboard)
\begin{frame}{Le tableau de bord — 48 validations, 0 rouge}
  \centering
  \includegraphics[width=0.70\textwidth]{fig_house_dashboard.png}
\end{frame}
...
\begin{frame}{Ce que ce deck prouve — et la suite}
  ...
  \tuile{okgreen}{48 / 0}{validations / rouge\\tout re-jouable}\\[5mm]
  {\footnotesize
  ...
  {\scriptsize\color{black!60} suites naturelles : le Sénat sur le même modèle ·
   OCR des H/T/O scannés pour doubler la cohorte de cohérence (45 → 106 membres) ·
