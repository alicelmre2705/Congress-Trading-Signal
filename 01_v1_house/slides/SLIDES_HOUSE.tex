  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \colorbox{alertred!10}{\parbox{0.96\linewidth}{\scriptsize
        \textbf{\textcolor{alertred}{découverte — la source s'efface}} :
        le site \textbf{purge} les vieux index (rétention $\approx$ 6 ans).
        L'index 2014 re-téléchargé : \textbf{11} dépôts, contre \textbf{2\,788}
        dans notre instantané.\\[0.8mm]
        → le recensement V1 = \textbf{union} site $\cup$ instantané archivé
        (\co{reference/index\_instantane/}), source tracée ligne à ligne.}}\\[2mm]
...
\begin{frame}{Le \texttt{DocID} (2/4) — le préfixe dit-il vrai ? Vérifié sur 23\,246 PDF}
  \centering
  ...
  \begin{tabular}{l cc}
    ...
    préfixe scanné (\co{8}/\co{9}) &
      \textcolor{alertred}{\textbf{0}} & \textcolor{okgreen}{\textbf{2\,425}} \\
    \bottomrule
  \end{tabular}}\\[4mm]
  \badge{okgreen}{0 désaccord sur 23\,246 documents — validation L4}\\[2mm]
...
\begin{frame}{Résultat — les portefeuilles se recoupent, et les trous s'expliquent}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \textbf{et le 21 \% inexpliqué ?} attribué un par un : non-filers
      (déclarent tout sauf leurs trades), fonds sous le seuil des 1\,000 \$…
      et \textbf{0 blind trust} (C-d = C-e = 0) — le trust, lui, fait
      \emph{disparaître} : Dean Phillips entre avec \textbf{284} positions,
      sort avec \textbf{1} ligne opaque
      « \emph{Qualified Blind Trust [EQ]} », sans ticker.}
    \end{column}
  \end{columns}
\end{frame}

% ── H16 · Le tableau de bord ── (MANIFEST.json : 41 validations, 0 rouge ; fig_house_dashboard)
\begin{frame}{Le tableau de bord — 41 validations, 0 rouge}
  \centering
  \includegraphics[width=0.80\textwidth]{fig_house_dashboard.png}
\end{frame}

% ── H17 · Conclusion ── (comptes MANIFEST : 161 076 signal · 198 615 holdings · 23 246 PDF)
\begin{frame}{Ce que ce deck prouve — et la suite}
  ...
  \tuile{grisC}{23\,246}{PDF archivés\\+ index irremplaçables}\hfill
  \tuile{okgreen}{41 / 0}{validations / rouge\\tout re-jouable}\\[5mm]
  {\footnotesize
  \badge{helec}{1. source exhaustive et archivée — le site purge, pas nous}\\[1.5mm]
  \badge{helec}{2. tri lisible/scanné mesuré (0 désaccord) — l'OCR au strict nécessaire}\\[1.5mm]
  \badge{helec}{3. la chronologie de chaque portefeuille se recoupe — H + P ⊇ T à 78,7 \%}}\\[4mm]
  {\scriptsize\color{black!60} suites naturelles : le Sénat sur le même modèle ·
   OCR des H/T/O scannés pour élargir la cohorte de cohérence ·
