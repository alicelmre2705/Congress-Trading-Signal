...
House 2014-2026 $\cdot$ notebook 10 (indépendant, chaque chiffre sort d'une cellule exécutée) $\cdot$ 11 années civiles vs SPY, seuil $t_{10} \approx 1{,}81$}
\end{center}
\vspace{1mm}
...
\node[b={rouge}{25.8cm}] at (13.4, 4.4)
  {\textbf{le résultat en une phrase} : aucun run ne bat le SPY significativement ($\max t = 0{,}85$, l'effet photo B) --- et les trades O/A/T
   récupérés pèsent \textbf{négativement} ($-2{,}2\,\%$/an apparié) : compléter les données a rendu le test plus honnête, pas la stratégie meilleure};

\node[font=\scriptsize, text=grisf] at (13.4, 3.0)
  {p.\,2 --- ce que les corrections ajoutent \quad$\cdot$\quad p.\,3 --- ce que ça change pour un membre \quad$\cdot$\quad
   p.\,4 --- le verdict \quad$\cdot$\quad p.\,5 --- d'où vient le $+6{,}6\,\%$ de B \quad$\cdot$\quad p.\,6 --- conclusion et limites};
...
\end{tabular}
\end{center}
{\scriptsize $^{\dagger}$ après tentative de téléchargement — fonds monétaires en tête (SPAXX, FZDXX, FDRXX\ldots). $^{\ddagger}$ 490 appariements : 9 lignes matchent 2 lots PTR du même jour.\\
...
\begin{center}
{\Large\bfseries 3 $\cdot$ Qu'est-ce que ça change pour un membre --- et donc pour le classement ?}\\[0.5mm]
...
\begin{center}\includegraphics[width=0.78\linewidth]{figures/fig_10_sharpe_shift.png}\end{center}
\end{minipage}

\newpage
% ══════════ PAGE 4 — LE VERDICT CHANGE-T-IL ? ══════════
\begin{center}
{\Large\bfseries 4 $\cdot$ Le verdict change-t-il ?}\\[0.5mm]
...
\begin{center}
{\Large\bfseries 5 $\cdot$ D'où vient (et ne vient pas) le $+6{,}6\,\%$ de B ?}\\[0.5mm]
...
% ══════════ PAGE 6 — QUE CONCLURE ? ══════════
\begin{center}
{\Large\bfseries 6 $\cdot$ Que conclure --- et pourquoi y croire ?}
\end{center}

\noindent\begin{minipage}[t]{0.47\textwidth}\vspace{0pt}
\textbf{\color{rouge}Verdict}
\begin{itemize}
\item[--] \textbf{pas d'alpha démontrable} : $\max t = 0{,}85$, effets appariés nuls, strates $\le 0{,}60$ --- robuste au poids, à la strate, et au périmètre (et il se renforce quand on complète les données : D $-1{,}6\,\%$/an) ;
\item[--] les corrections rendent la mesure \textbf{plus honnête}, pas plus rentable : $\beta$ $0{,}99 \to 0{,}85$ (le témoin cachait du beta), et l'effet O/A/T est négatif ;
...
\vspace{1mm}
\textbf{La valeur du travail, même sous le null} : le pipeline de données est réparé --- 437 traders au lieu de 305, des ventes exécutables, un patrimoine d'entrée mesuré --- et sert telle quelle à toute stratégie future (ticker, notebook 11).
\end{minipage}
