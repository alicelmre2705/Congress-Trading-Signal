...
% ── II.4 · Chambre scannée ── (§4.5 ; census/motifs slide OCR)
\begin{frame}{Le chemin Chambre (4/4) — le document : PTR scanné}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}\centering
      \fcolorbox{hocr}{white}{\includegraphics[height=57mm]{sources/house_ocr.png}}\\[0.6mm]
      {\scriptsize scan tapé, \textbf{couché}, tampon \emph{HAND DELIVERED} (Anna P.\ Luna)}
    \end{column}
    \begin{column}{0.43\textwidth}
      \badgel{hocr}{93\,261 transactions — 55,2\,\% du corpus}\\[2.5mm]
      {\footnotesize
      · une \textbf{image}, pas de texte : formulaire à \textbf{cases cochées}
        (type, fourchette A→K)\\[1.5mm]
      · souvent \textbf{tourné à 90°}, déposé à la main\\[1.5mm]
      · \textcolor{alertred}{le cas extrême} : le \textbf{manuscrit cursif} →}\\[1.5mm]
      \fcolorbox{alertred}{white}{\includegraphics[height=26mm]{slides/fig_pdf_manuscrit_clay.png}}\\[0.6mm]
      \badge{alertred}{écarté — règle rejouable (→ étape 5)}
    \end{column}
  \end{columns}
\end{frame}

% ── II.5 · Le site du Sénat ── (capture réelle efdsearch.senate.gov ; 2 150 comptés)
\begin{frame}{Le chemin Sénat (1/3) — le site eFD et sa barrière}
  \begin{columns}[T]
    \begin{column}{0.64\textwidth}\centering
    ...
    \begin{column}{0.34\textwidth}
      ...
      · ensuite : recherche paginée (type PTR) → \textbf{2\,150} pages HTML
        mises en cache\\[1.5mm]
      · le \textbf{papier} (images .gif) est servi \textbf{hors barrière}}
    \end{column}
  \end{columns}
\end{frame}

% ── II.6 · Sénat électronique ── (§4.4 « dans la source »)
\begin{frame}{Le chemin Sénat (2/3) — le document : déclaration web}
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}\centering
      \fcolorbox{selec}{white}{\includegraphics[width=\linewidth]{sources/senate_digital.png}}
    \end{column}
    \begin{column}{0.36\textwidth}
      \badge{selec}{13\,026 transactions — 7,7\,\%}\\[3mm]
      {\footnotesize
      · pas de PDF : une \textbf{page web} derrière la barrière CSRF\\[1.5mm]
...
% ── II.7 · Sénat papier ── (§4.6 « dans la source »)
\begin{frame}{Le chemin Sénat (3/3) — le document : papier scanné}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}\centering
      \fcolorbox{socr}{white}{\includegraphics[height=64mm]{sources/senate_ocr.png}}
    \end{column}
    \begin{column}{0.43\textwidth}
      \badgel{socr}{3\,985 transactions — 2,4\,\% du corpus}\\[3mm]
      {\footnotesize
      · images \co{.gif} servies hors barrière — \textbf{déjà droites}
        (pas de redressement)\\[1.5mm]
...
% ── S13a · Trois familles de scans (galerie de scénarios) ── (census 74/1575/775 ; §6.6 88/65/13)
\begin{frame}{Étape 5 — trois familles de scans, trois résultats}
  \centering
  {\footnotesize sur \textbf{2\,424 scans} recensés, ce qu'on arrive à lire dépend
   du type de document :}\\[1.5mm]
  % Ligne 1 : tapé droit
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}\centering
      \fcolorbox{okgreen}{white}{\includegraphics[height=16mm]{slides/thumb_scan_droit.png}}
    \end{column}
    \begin{column}{0.40\textwidth}
      {\footnotesize\textbf{\color{okgreen}Tapé, droit} — 74 scans}\\[0.5mm]
      \badge{okgreen}{✓ lu directement}\\[0.5mm]
      {\scriptsize corroboré par Quiver à \textbf{88\,\%}}
    \end{column}
  \end{columns}
  \vspace{1.2mm}
  % Ligne 2 : tapé tourné
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}\centering
      \fcolorbox{helec}{white}{\includegraphics[height=16mm]{slides/thumb_scan_tourne.png}}
    \end{column}
    \begin{column}{0.40\textwidth}
      {\footnotesize\textbf{\color{helec}Tapé, couché} — 1\,575 scans (le gros)}\\[0.5mm]
      \badge{helec}{↻ redressé, puis lu}\\[0.5mm]
      {\scriptsize corroboré à \textbf{65\,\%} — cf.\ slide suivante}
    \end{column}
  \end{columns}
  \vspace{1.2mm}
  % Ligne 3 : manuscrit
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}\centering
      \fcolorbox{alertred}{white}{\includegraphics[height=16mm]{slides/thumb_scan_manuscrit.png}}
    \end{column}
    \begin{column}{0.40\textwidth}
      {\footnotesize\textbf{\color{alertred}Manuscrit} — 775 scans}\\[0.5mm]
      \badge{alertred}{✗ dates non fiables → écarté}\\[0.5mm]
      {\scriptsize corroboré à \textbf{13\,\%} seulement}
    \end{column}
  \end{columns}
\end{frame}

% ── S13b · Ligne par ligne : récupéré, corrigé, écarté ── (echantillon match ; schema.py ; RQ §6.2 2540)
\begin{frame}{Étape 5 — ligne par ligne : récupéré, corrigé, ou écarté}
  \begin{columns}[T]
    \begin{column}{0.40\textwidth}\centering
      \includegraphics[width=\linewidth]{slides/fig_deskew_rotations.png}\\[1mm]
      {\scriptsize\color{black!60} \textbf{redresser} d'abord : 525/547 scans arrivent
       couchés → on choisit la bonne rotation avant l'OCR}
    \end{column}
    \begin{column}{0.58\textwidth}
      {\footnotesize puis, pour chaque ligne lue :}\\[1.5mm]
      \colorbox{okgreen!12}{\parbox{0.96\linewidth}{\scriptsize
        \textbf{✓ lu correctement} — la majorité\\
        \co{MICROSOFT CORP} → \textbf{MSFT}, 27/12/2019 (identique à Quiver)}}\\[1.2mm]
      \colorbox{helec!12}{\parbox{0.96\linewidth}{\scriptsize
        \textbf{⚙ ticker absent → récupéré} par LLM\\
        \co{« NEENAH PAPER »} (nom lisible, pas de symbole) → \textbf{NP}
        — \textbf{2\,540 lignes} ainsi récupérées}}\\[1.2mm]
      \colorbox{warnorange!16}{\parbox{0.96\linewidth}{\scriptsize
        \textbf{⚙ date suspecte → corrigée} au PDF\\
        Khanna / Costco : l'OCR lit \co{2021-09-31} (le 31 sept.\ n'existe pas)
        → relu → \textbf{2021-08-16}}}\\[1.2mm]
      \colorbox{alertred!12}{\parbox{0.96\linewidth}{\scriptsize
        \textbf{✗ non récupérable → écarté / flaggé}\\
        manuscrit cursif ou charabia OCR — \textbf{on n'invente jamais} une date}}
    \end{column}
  \end{columns}
  \vspace{1mm}
  {\scriptsize\color{black!60} bilan : \textbf{93\,261 lignes} (Khanna 44\,\%, trio de tête 77\,\%) ·
   \textbf{582 manuscrits écartés}, règle rejouable (exceptions : 3 déposants + 33 docs curés)}
\end{frame}
