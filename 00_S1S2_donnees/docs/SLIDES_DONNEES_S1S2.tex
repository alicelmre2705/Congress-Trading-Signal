...
% ── S2 · Chambre : site → ZIP → index XML ── (fusion ex-C1+C2)
\begin{frame}{La Chambre --- tout part d'un index XML public}
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      \centering
      \begin{tikzpicture}[node distance=5mm]
        \node[step=grisC, text width=4.6cm] (s) {le site public du \emph{House Clerk}\\{\scriptsize une archive par année, 2014 → 2026}};
        \node[step=helec, text width=4.6cm, below=of s] (z) {on télécharge l'archive\\[0.5mm]\co{2024FD.zip}};
        \node[step=helec, text width=4.6cm, below=of z] (x) {on la dézippe → l'\textbf{index annuel}\\[0.5mm]\co{2024FD.xml}};
        \node[step=okgreen, text width=4.6cm, below=of x] (i) {un bloc \co{<Member>}\\par \textbf{dépôt déclaré}};
        \draw[arr] (s) -- (z); \draw[arr] (z) -- (x); \draw[arr] (x) -- (i);
      \end{tikzpicture}\\[1.5mm]
      {\scriptsize \href{https://disclosures-clerk.house.gov/FinancialDisclosure}%
       {\texttt{disclosures-clerk.house.gov}}} {\scriptsize\color{black!55}(cliquable)}
    \end{column}
    \begin{column}{0.56\textwidth}\centering
      \fcolorbox{black!25}{white}{\includegraphics[height=36mm]{slides/site_house_clerk.png}}\\[1.5mm]
      \includegraphics[height=30mm]{slides/fichier_index_xml.png}
    \end{column}
  \end{columns}
\end{frame}
...
% ── S5 · Le lisible et ses pièges ── (fusion ex-C5+C5bis)
\begin{frame}{Le monde lisible : propre… sauf les multilignes}
  \begin{columns}[T]
    ...
    \begin{column}{0.38\textwidth}
      \badge{helec}{DocID en \co{2…} → couche texte}\\[2mm]
      {\footnotesize
      · le ticker est \textbf{imprimé} : \co{(BA)}, le type d'actif : \co{[ST]},
        2 dates + fourchette\\[1.5mm]
      · mais le PDF n'est \textbf{pas un tableau régulier} : le \textbf{montant} déborde
        sur la ligne suivante, un \textbf{libellé} court sur 4 lignes}\\[2.5mm]
      \colorbox{helec}{\parbox{0.95\linewidth}{\footnotesize\color{white}\centering
        → on \textbf{reconstruit} chaque transaction\\(recollage) avant de la lire}}\\[2mm]
      {\scriptsize\color{black!60} le montant coupé à lui seul : \textbf{7\,996} lignes réparées}
    \end{column}
  \end{columns}
\end{frame}
...
\subsection{Le Sénat}
% ── S7 · Sénat : accès + électronique ── (fusion ex-II.5+II.6)
\begin{frame}{Le Sénat --- un portail à apprivoiser, un cas électronique propre}
  \centering
  \resizebox{0.94\textwidth}{!}{%
  \begin{tikzpicture}[node distance=6mm]
    \node[step=alertred, text width=2.4cm] (a) {\textbf{Accord}\\+ jeton d'accès (CSRF)};
    ...
    \node[bande=selec, text width=2.6cm, right=8mm of c, yshift=7mm] (e1) {\textbf{texte} → page HTML};
    \node[bande=socr, text width=2.6cm, right=8mm of c, yshift=-7mm] (e2) {\textbf{image} → scan};
    \draw[arr] (a)--(b); \draw[arr] (b)--(c);
    \draw[arr] (c.east) to[out=20,in=180] (e1.west);
    \draw[arr] (c.east) to[out=-20,in=180] (e2.west);
  \end{tikzpicture}}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}\centering
      ...
      {\scriptsize\color{black!60} le portail \textbf{eFD} (\emph{electronic Financial Disclosure}) :
       table propre, ticker cliquable}
    \end{column}
    \begin{column}{0.42\textwidth}
      \badge{selec}{13\,026 transactions électroniques}\\[2.5mm]
      {\footnotesize
      · \textbf{1\,705} rapports « Sénateur × PTR » listés\\[1.2mm]
      · un gabarit de table \textbf{stable sur 13 ans} (détail en Partie 2)\\[1.2mm]
  ...
  \begin{tikzpicture}[node distance=3mm]
    ...
    \node[bande=alertred, text width=13.6cm] (val) at (0,-8.45)
      {\textbf{validation} — Quiver (3 niveaux : existence / date / champs) + sources indépendantes
       + tables « golden » figées (SHA-256) — \textbf{jamais réinjectées}};
...
% ── S16 · Cascade ticker + GICS→ETF ── (ex-Étape 7 reprise)
\begin{frame}{Un ticker, un secteur, un ETF : la cascade}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}\footnotesize
