...
\newcommand{\pipefig}[1]{%
  ...
  \def\opF{0.16}\def\opG{0.16}\def\opH{0.16}\def\opI{0.16}%
  \ifnum#1=0 \def\opA{1}\def\opB{1}\def\opC{1}\def\opD{1}\def\opE{1}%
             \def\opF{1}\def\opG{1}\def\opH{1}\def\opI{1}\fi
  ...
  \ifnum#1=7 \def\opG{1}\fi  \ifnum#1=8 \def\opH{1}\fi  \ifnum#1=9 \def\opI{1}\fi
  \begin{tikzpicture}[node distance=3mm, every node/.style={transform shape}]
    ...
    \begin{scope}[opacity=\opB, text opacity=\opB]
      \node[step=helec, text width=2.6cm] (hpdf) at (-5.3,-1.35) {\textbf{PDF lisible}\\couche texte};
      \node[step=hocr, text width=2.6cm] (hscan) at (-1.9,-1.35) {\textbf{scan}\\image};
      \node[step=selec, text width=2.6cm] (shtml) at (1.9,-1.35) {\textbf{page HTML}\\texte};
      \node[step=socr, text width=2.6cm] (sgif) at (5.3,-1.35) {\textbf{papier}\\image};
    ...
    \begin{scope}[opacity=\opC, text opacity=\opC]
      \node[step=helec, text width=2.6cm] (hpar) at (-5.3,-2.85)
        {\textbf{parsing}\\[0.3mm]{\small\textbf{\color{helec}58\,728}}};
      \node[step=hocr, text width=2.6cm] (hvis) at (-1.9,-3.55)
        {\textbf{OCR Vision}\\[0.3mm]{\small\textbf{\color{helec}93\,261}}};
      \node[step=selec, text width=2.6cm] (spar) at (1.9,-2.85)
        {\textbf{tableaux}\\[0.3mm]{\small\textbf{\color{selec}13\,026}}};
      \node[step=socr, text width=2.6cm] (svis) at (5.3,-2.85)
        {\textbf{OCR Vision}\\[0.3mm]{\small\textbf{\color{selec}3\,985}}};
    ...
    \begin{scope}[opacity=\opD, text opacity=\opD]
      \node[bande=grisC, text width=12.6cm] (ident) at (0,-4.7)
        {\textbf{identité} --- nom libre → identifiant officiel de l'élu \;·\; \textbf{100\,\%}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-4.7) {I.4 Identité};
    \end{scope}
    \begin{scope}[opacity=\opE, text opacity=\opE]
      \node[bande=dataC, text width=12.6cm] (tick) at (0,-5.5)
        {\textbf{ticker} --- le nom de l'actif → son symbole boursier \;·\; \textbf{84,1 / 77,9\,\%}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-5.5) {I.5 Ticker};
    \end{scope}
    \begin{scope}[opacity=\opF, text opacity=\opF]
      \node[bande=dataC, text width=12.6cm] (sect) at (0,-6.3)
        {\textbf{secteur} --- GICS → ETF \;·\; \textbf{79,8 / 70,4\,\%} \;·\; montant → \emph{milieu de fourchette}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-6.3) {I.6 Secteur};
    \end{scope}
    \begin{scope}[opacity=\opG, text opacity=\opG]
      \node[bande=grisC, text width=12.6cm] (ded) at (0,-7.1)
        {\textbf{dédup} --- l'empreinte à 7 champs : 170\,920 lignes brutes →
         \textbf{TABLE DU CORPUS : 169\,000 uniques × 28 col.}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-7.1) {I.7 Dédup};
    \end{scope}
    % ---------- I.8 · Entonnoir ----------
    \begin{scope}[opacity=\opH, text opacity=\opH]
      \node[bande=okgreen, text width=12.6cm] (ent) at (0,-8.0)
        {\textbf{entonnoir} --- on ne retire que l'avéré inutilisable : \textbf{−34\,536}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-8.0) {I.8 Entonnoir};
    \end{scope}
    % ---------- I.9 · Table finale ----------
    \begin{scope}[opacity=\opI, text opacity=\opI]
      \node[bande=okgreen, fill=okgreen!16, text width=12.6cm] (fin) at (0,-8.85)
        {\textbf{TABLE DE RECHERCHE} --- \textbf{134\,464 × 36}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-8.85) {I.9 Table};
    \end{scope}
    % ---------- les flèches ----------
    \begin{scope}[opacity=0.30]
      ...
      \draw[arr] (sect) -- (ded); \draw[arr] (ded) -- (ent); \draw[arr] (ent) -- (fin);
    \end{scope}
  \end{tikzpicture}}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \colorbox{black!6}{\parbox{\dimexpr\linewidth-8pt}{\tiny\vspace{0.4mm}
        \textbf{Chambre et Sénat} : une slide chacun \textbf{là où ils diffèrent}
        (I.1, I.2, I.3), une seule slide \textbf{là où c'est identique}\vspace{0.4mm}}}\\[2mm]
      \colorbox{black!6}{\parbox{\dimexpr\linewidth-8pt}{\tiny\vspace{0.4mm}
...
\begin{frame}[plain]
  ...
  {\color{black!55}\large De $\sim$10\,000 documents publics hétérogènes à une seule table}\\[8mm]
  {\footnotesize\color{black!55} le \textbf{pipeline} revient avant chaque étape :
   on verra toujours \textbf{où l'on est}}
\end{frame}

% ── I.0 · Le pipeline complet ──
\begin{frame}{I.0 · Le pipeline complet --- tout le trajet de la donnée}
  \centering
  {\scriptsize\color{black!55} ce schéma \textbf{revient avant chaque étape}, l'étape en
   cours allumée \;·\; \textbf{OCR} = lecture automatique d'une image (I.2d)}\\[0.5mm]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \begin{columns}[T]
        \begin{column}{0.48\linewidth}\centering
          ...
          \badge{helec}{lisible}\\[0.2mm]{\scriptsize PDF à couche texte\\\textbf{\mbox{71\,\%}}}
        \end{column}
        \begin{column}{0.48\linewidth}\centering
          \fcolorbox{hocr}{white}{\includegraphics[width=0.9\linewidth,height=17mm,keepaspectratio]{sources/house_ocr.png}}\\[0.5mm]
          \badgel{hocr}{scanné}\\[0.2mm]{\scriptsize PDF image\\\textbf{\mbox{29\,\%}}}
        \end{column}
      \end{columns}
    \end{column}
  \end{columns}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{columns}[T]
        \begin{column}{0.48\linewidth}\centering
          ...
          \badge{selec}{page web}\\[0.2mm]{\scriptsize table propre\\\textbf{1\,780}}
        \end{column}
        \begin{column}{0.48\linewidth}\centering
          \fcolorbox{socr}{white}{\includegraphics[width=0.9\linewidth,height=17mm,keepaspectratio]{slides/senat_papier_droit.png}}\\[0.5mm]
          \badgel{socr}{papier}\\[0.2mm]{\scriptsize formulaire scanné\\\textbf{373}}
        \end{column}
      \end{columns}
    \end{column}
  \end{columns}
...
\begin{frame}{I.2c · Bilan du tri : quatre sous-corpus, quatre extracteurs}
  ...
  \begin{columns}[T]
    \begin{column}{0.24\textwidth}\centering
      \colorbox{helec}{\parbox{0.92\linewidth}{\centering\vspace{0.4mm}{\color{white}\scriptsize\textbf{Chambre · lisible}}\vspace{0.4mm}}}\\[1mm]
      {\scriptsize PDF à couche texte\\→ \textbf{parsing} (I.3a)}
    \end{column}
    \begin{column}{0.24\textwidth}\centering
      \colorbox{hocr}{\parbox{0.92\linewidth}{\centering\vspace{0.4mm}{\color{black!80}\scriptsize\textbf{Chambre · scanné}}\vspace{0.4mm}}}\\[1mm]
      {\scriptsize PDF image\\→ \textbf{OCR} (I.3b)}
    \end{column}
    \begin{column}{0.24\textwidth}\centering
      \colorbox{selec}{\parbox{0.92\linewidth}{\centering\vspace{0.4mm}{\color{white}\scriptsize\textbf{Sénat · page web}}\vspace{0.4mm}}}\\[1mm]
      {\scriptsize table HTML\\→ \textbf{lecture} (I.3c)}
    \end{column}
    \begin{column}{0.24\textwidth}\centering
      \colorbox{socr}{\parbox{0.92\linewidth}{\centering\vspace{0.4mm}{\color{black!80}\scriptsize\textbf{Sénat · papier}}\vspace{0.4mm}}}\\[1mm]
      {\scriptsize image\\→ \textbf{OCR} (I.3d)}
    \end{column}
  \end{columns}
  \vspace{4mm}
...
\pipeslide{3}{I.3 · Extraction --- on est ici}%
  ...
  {les \textbf{4 voies} lues : 58\,728 + 93\,261 + 13\,026 + 3\,985}
...
\begin{frame}{I.3a · Extraction Chambre · texte --- un PDF n'est pas un tableau}
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      \badge{helec}{58\,728 transactions}\\[2mm]
...
\begin{frame}{I.3c · Extraction Sénat · texte --- une table déjà propre, stable sur 13 ans}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}\centering
      \fcolorbox{selec}{white}{\includegraphics[width=0.96\linewidth,height=0.36\textheight,keepaspectratio]{sources/senate_digital.png}}\\[0.5mm]
      {\scriptsize\color{black!60} la déclaration électronique : une vraie table, ticker
       cliquable --- rien à reconstruire}
    \end{column}
    \begin{column}{0.42\textwidth}
      \badge{selec}{13\,026 transactions}\\[2mm]
      {\footnotesize \textbf{1.} une \textbf{page par dépôt}, mise en cache\\[1mm]
       \textbf{2.} \textbf{lecture des tableaux} : les colonnes repérées par leurs
       intitulés\\[1mm]
       \textbf{3.} rien d'autre --- le gabarit ne change pas}\\[2.5mm]
      {\scriptsize\color{black!60} \textbf{les 9 mêmes colonnes} sur toutes les pages
       ...
       non-coté, pas le format}
    \end{column}
  \end{columns}
  \vspace{1.5mm}
  \diff{là où la \chambre{} impose de \textbf{reconstruire} un PDF (I.3a), le \senat{}
   électronique se lit \textbf{directement} --- même exigence pourtant :
   \textbf{déterministe, sans IA}, zéro échec silencieux, chaque dépôt sans transaction
   consigné.}
\end{frame}
...
\pipeslide{7}{I.7 · Dédup --- on est ici}%
  ...
  {\textbf{169\,000} transactions uniques (170\,920 lignes brutes)}
...
\pipeslide{8}{I.8 · Entonnoir --- on est ici}%
  ...
  {\textbf{134\,464} transactions exploitables --- 79,6\,\% du corpus conservé}
...
\begin{frame}{A2 --- Le portail du Sénat : ce que rend la recherche}
  ...
  {\footnotesize la liste réelle des dépôts (I.1b) --- \textbf{un lien par dépôt} :
   chacun s'ouvre en \textbf{page web} ou en \textbf{scan}, c'est là que se décide la
