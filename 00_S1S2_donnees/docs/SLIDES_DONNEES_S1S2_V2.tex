...
% en PARTIE I — vision globale d'abord (I.0), puis 8 étapes numérotées, Chambre et Sénat
% traités ensemble à chaque étape (côte à côte quand ils diffèrent).
% FUSION 2026-07-17 (demande d'Alice) : Acquisition et Tri ne font plus qu'UNE étape (I.1),
% et l'on suit la Chambre EN ENTIER (I.1a acquisition → I.1b tri) avant le Sénat EN ENTIER
% (I.1c → I.1d) — l'alternance précédente « coupait la réflexion ». Tout l'aval a décalé
% d'un cran (ex-I.3 Extraction → I.2 … ex-I.9 Table → I.8) : 9 étapes → 8.
...
% fusion Acquisition+Tri (2026-07-17), l'étape I.1 en allume deux. Correspondance :
...
\newcommand{\pipefig}[1]{%
  ...
  \begin{tikzpicture}[node distance=3mm, every node/.style={transform shape}]
    ...
    \begin{scope}[opacity=\opA, text opacity=\opA]
      ...
      % une seule étiquette pour l'étape I.1 : elle couvre l'acquisition ET le tri,
      % d'où sa position entre les deux rangées.
      \node[font=\tiny\bfseries, text=helec!80, align=left] at (-7.9,-0.62)
        {I.1 Acquisition\\\& Tri};
    \end{scope}
    % ---------- I.1 · Tri (même étape que l'acquisition : \opA et \opB s'allument ensemble)
    ...
    \end{scope}
    % ---------- I.2 · Extraction ---------- (chiffres BRUTS, avant dédup : leur somme = 170 920)
    \begin{scope}[opacity=\opC, text opacity=\opC]
      ...
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-3.1) {I.2 Extraction};
    \end{scope}
    % ---------- I.3 à I.6 · le tronc commun ----------
    \begin{scope}[opacity=\opD, text opacity=\opD]
      ...
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-4.95) {I.3 Identité};
    \end{scope}
    \begin{scope}[opacity=\opE, text opacity=\opE]
      ...
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-5.75) {I.4 Ticker};
    \end{scope}
    \begin{scope}[opacity=\opF, text opacity=\opF]
      ...
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-6.55) {I.5 Secteur};
    \end{scope}
    % ---------- la sortie du pipeline (visible en I.0, I.5 et I.6 : c'est la frontière)
    ...
    \begin{scope}[opacity=\opG, text opacity=\opG]
      ...
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-8.65) {I.6 Dédup};
    \end{scope}
    % ---------- I.7 · Entonnoir ---------- (les LIGNES : le seul entonnoir qui touche la table)
    \begin{scope}[opacity=\opH, text opacity=\opH]
      \node[bande=okgreen, text width=12.6cm] (ent) at (0,-9.45)
        {\textbf{entonnoir des LIGNES} --- on ne retire que l'avéré inutilisable :
         \textbf{−34\,536}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-9.45) {I.7 Entonnoir};
    \end{scope}
    % ---------- I.8 · Table finale ---------- (LE LIVRABLE : le pipeline S1/S2 s'arrête ici)
    \begin{scope}[opacity=\opI, text opacity=\opI]
      \node[bande=okgreen, fill=okgreen!16, text width=12.6cm] (fin) at (0,-10.3)
        {\textbf{TABLE DE RECHERCHE} --- \textbf{134\,464 × 36}
         \;{\tiny(le livrable --- aucune colonne de prix)}};
      \node[font=\tiny\bfseries, text=helec!80] at (-7.9,-10.3) {I.8 Table};
    \end{scope}
    % ---------- côté recherche : le 2e entonnoir, APRÈS la livraison ----------
    % Source : FICHE_NETTOYAGE_BACKTEST_V2 §« L'entonnoir des tickers » — « du symbole au prix
    % (côté recherche, notebooks 05/06) ». Il ne retire AUCUNE ligne : les non couvertes
    % restent dans la table, flaguées. Rattaché à \opH = la 2e moitié de l'étape Entonnoir.
    \draw[dashed, draw=black!35, line width=0.6pt] (-8.4,-10.82) -- (7.0,-10.82);
    \node[font=\tiny\itshape, text=black!50, anchor=west] at (-8.4,-11.04)
      {côté recherche --- \textbf{notebooks 05/06}, une fois la table livrée};
    \begin{scope}[opacity=\opH, text opacity=\opH]
      \node[bande=dataC, dashed, text width=12.6cm] (sym) at (0,-11.6)
        {\textbf{entonnoir des SYMBOLES} --- 4\,618 → 3\,350 servis par Yahoo →
         \textbf{3\,289} exploitables\\[0.3mm]
         {\tiny → \textbf{118\,029} trades \textbf{backtestables} (≈\,88\,\%) \;·\;
          \mbox{\textbf{aucune ligne retirée}} : les autres restent dans la table, flaguées}};
      \node[font=\tiny\bfseries, text=dataC!80] at (-7.9,-11.6) {prix};
    \end{scope}
    % ---------- les flèches ----------
    \begin{scope}[opacity=0.30]
      ...
      \draw[arr] (ded) -- (ent); \draw[arr] (ent) -- (fin);
      % le 2e entonnoir part DE la table livrée, en aval : pointillés = hors pipeline
      \draw[arr, dashed] (fin) -- (sym);
    \end{scope}
  \end{tikzpicture}}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tabular}{@{}p{0.55cm}>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}p{3.4cm}@{}}
        ...
        \etape{I.1}{Acquisition \& Tri}{la Chambre en entier, puis le Sénat en entier}
        \etape{I.2}{Extraction}{4 voies · les 2 dates · le corpus}
        \etape{I.3}{Identité}{le nom → l'élu officiel}
        \etape{I.4}{Tickers}{le symbole boursier}
        \etape{I.5}{Secteurs}{GICS → ETF}
        \etape{I.6}{Dédup}{l'empreinte 7 champs}
        \etape{I.7}{Entonnoir}{ce qu'on retire, et pourquoi}
        \etape{I.8}{Table finale}{à quoi ressemble la valeur}
      \end{tabular}}
    \end{column}
    \begin{column}{0.48\textwidth}
...
% ════ I.1 · ACQUISITION & TRI ════
\pipeslide{1}{I.1 · Acquisition \& Tri --- on est ici}%
...
\begin{frame}{I.1a · Acquisition Chambre --- un index annuel qui fait autorité}
  ...
  {\footnotesize \textbf{35\,315} dépôts de tous types · \textbf{13 années} couvertes ·
   téléchargement \textbf{automatisé}}
\end{frame}

% ── I.1b · Tri Chambre ──
\begin{frame}{I.1b · Tri Chambre --- douze types dans l'index, un seul porte le signal}
  \begin{columns}[c]
...
% ── I.1c · Acquisition Sénat ──
\begin{frame}{I.1c · Acquisition Sénat --- pas d'index : un portail à interroger}
  \begin{columns}[T]
...
% ── I.1d · Tri Sénat ──
\begin{frame}{I.1d · Tri Sénat --- pas de lettres à lire : le type se demande au portail}
  \begin{columns}[c]
...
% ── I.1e · Les quatre sous-corpus ──
\begin{frame}{I.1e · Bilan du tri : quatre sous-corpus, quatre extracteurs}
  \centering
  \vspace{1mm}
  ...
  \begin{columns}[T]
    \begin{column}{0.24\textwidth}\centering
      ...
      {\scriptsize \textbf{texte}\\→ \textbf{parsing} (I.2a)}
    \end{column}
    \begin{column}{0.24\textwidth}\centering
    ...
    \begin{column}{0.24\textwidth}\centering
      ...
      {\scriptsize \textbf{image}\\→ \textbf{OCR} (I.2d)}
    \end{column}
  \end{columns}
  \vspace{2.5mm}
  \diff{texte ou image, \textbf{comment le sait-on} ? --- à la \chambre{} on
   \textbf{essaie de lire} le PDF (du texte en sort → c'est du texte, sinon une image) ;
   au \senat{} le \textbf{lien du portail} le dit déjà (\co{/ptr/} ou \co{/paper/}).}\\[2mm]
...
% ── I.1f · Le manuscrit ──
\begin{frame}{I.1f · Dans l'image, le vrai point dur n'est pas le scan : c'est le manuscrit}
  \centering
  ...
  \diff{le manuscrit existe des \textbf{deux} côtés --- mais il sera \textbf{écarté} à la
   ...
   \textbf{vérifiabilité} (I.2b et I.2d).}
\end{frame}

% ════ I.2 · EXTRACTION ════
\pipeslide{2}{I.2 · Extraction --- on est ici}%
...
% ── I.2a · Extraction Chambre, texte ──
\begin{frame}{I.2a · Extraction Chambre · texte --- un PDF n'est pas un tableau}
  \begin{columns}[T]
...
% ── I.2b · Extraction Chambre, image ──
\begin{frame}{I.2b · Extraction Chambre · image --- Vision lit tout, le manuscrit est écarté}
  \centering
...
% ── I.2c · Extraction Sénat, texte ──
\begin{frame}{I.2c · Extraction Sénat · texte --- une table déjà propre, stable sur 13 ans}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}\centering
    ...
    \begin{column}{0.42\textwidth}
      ...
      \colorbox{warnorange!12}{\parbox{0.96\linewidth}{\scriptsize\vspace{0.4mm}
        ...
        cette seule voie porte \textbf{93\,\%} du dédoublonnage (I.6)\vspace{0.4mm}}}
    \end{column}
  \end{columns}
  \vspace{1mm}
  \diff{là où la \chambre{} impose de \textbf{reconstruire} un PDF (I.2a), le \senat{}
...
% ── I.2d · Extraction Sénat, image ──
\begin{frame}{I.2d · Extraction Sénat · image --- le papier est gardé, faute de mètre externe}
  \begin{columns}[c]
    ...
    \begin{column}{0.66\textwidth}
      ...
      {\footnotesize \textbf{même moteur} qu'à la Chambre (I.2b) --- mais
...
% ── I.2e · Extraction : les deux dates ──
\begin{frame}{I.2e · Ce qu'on extrait de chaque ligne : deux dates, une seule est fiable}
  \centering
  \begin{columns}[T]
...
% ── I.2f · Le corpus extrait ── (ferme l'étape Extraction)
\begin{frame}{I.2f · Le corpus extrait : 169\,000 transactions --- dont $\sim$40\,\% de deux élus}
  \centering
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      ...
      {\footnotesize le papier n'est pas un détail : c'est \textbf{la majorité du
       corpus} --- d'où l'effort d'OCR de I.2b et I.2d}
    \end{column}
  \end{columns}
\end{frame}

% ════ I.3 · IDENTITÉ ════
\pipeslide{3}{I.3 · Identité --- on est ici}%
...
% ── I.3 · Identité ──
\begin{frame}{I.3 · Identité --- chaque ligne est rattachée à un élu officiel : \mbox{100\,\%}}
  \begin{columns}[T]
...
% ════ I.4 · TICKERS ════
\pipeslide{4}{I.4 · Tickers --- on est ici}%
...
% ── I.4 · Tickers ──
\begin{frame}{I.4a · Tickers --- du nom de l'actif au symbole boursier, par une cascade tracée}
  \begin{columns}[c]
...
% ── I.4b · le code type d'actif manque avant 2019 → le symbole fait foi ──
\begin{frame}{I.4b · Tickers --- avant 2019, pas de code d'actif à la Chambre : c'est le symbole qui décide}
  \begin{columns}[c]
    \begin{column}{0.57\textwidth}\centering
...
% ════ I.5 · SECTEURS ════
\pipeslide{5}{I.5 · Secteurs --- on est ici}%
...
% ── I.5 · Secteurs ──
\begin{frame}{I.5 · Secteurs --- chaque titre reçoit un secteur GICS, et son ETF}
  \begin{columns}[c]
...
% ════ I.6 · DÉDUP ════
\pipeslide{6}{I.6 · Dédup --- on est ici}%
...
% ── I.6 · Dédup ──
\begin{frame}{I.6 · Dédup --- une transaction = 7 champs : l'empreinte dédoublonne sans détruire}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}\footnotesize
      ...
      {\scriptsize elle exclut \textbf{délibérément} :\\
       · le \textbf{ticker} --- ajouté après coup (I.4), sa présence rendrait
    ...
    \begin{column}{0.43\textwidth}
      ...
      {\scriptsize\color{black!60} au même moment, à la lecture : les \textbf{réparations}
       ...
       3 dates) --- le \textbf{fichier extrait, lui, reste figé} (I.3)}
    \end{column}
  \end{columns}
\end{frame}

% ════ I.7 · ENTONNOIR ════
\pipeslide{7}{I.7 · Entonnoir --- on est ici}%
...
% ── I.7a · Entonnoir ──
\begin{frame}{I.7a · Entonnoir --- de 169\,000 à 134\,464 : on ne retire que l'avéré inutilisable}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \centering
    ...
    \begin{column}{0.40\textwidth}
      ...
      \colorbox{okgreen!10}{\parbox{0.96\linewidth}{\scriptsize
        ...
        fait foi, pas la case déclarée \emph{(pourquoi : I.4b)}}}
    \end{column}
  \end{columns}
  \vspace{1.5mm}
  \centering
...
% ── I.7b · Du symbole au prix ──
\begin{frame}{I.7b · Entonnoir (suite) --- du symbole au prix : 118\,029 trades valorisables}
  \begin{columns}[c]
    \begin{column}{0.52\textwidth}\centering
...
% ════ I.8 · TABLE FINALE ════
\pipeslide{8}{I.8 · Table finale --- on est ici}%
  {Au bout du pipeline : à quoi ressemble, concrètement, une ligne livrée ?}%
  {la table de recherche --- \textbf{134\,464 × 36}, limites écrites}

% ── I.8 · La table finale ── (ferme la Partie I)
\begin{frame}{I.8 · À quoi ressemble la valeur finale : 134\,464 lignes × 36 colonnes}
  \centering
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      {\scriptsize · \textbf{582 PTR manuscrits} écartés (7,1\,\% de l'index)\\
       · \textbf{biais de survie} prix (I.7b)\\
...
\begin{frame}{II.1 · L'univers --- 100\,\% des 8\,252 PTR officiels sont traités}
  ...
  \begin{tikzpicture}[node distance=6mm]
    ...
    \node[step=alertred, text width=3.2cm, minimum height=1.3cm, font=\footnotesize, right=6mm of c] (d)
      {\textbf{\large 582}\\{\scriptsize écartés --- manuscrits, règle écrite (I.2b)}};
    \draw[arr] (a) -- (b);
  \end{tikzpicture}}\\[3mm]
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \choixbox{les transactions hors PTR alimentent le \textbf{patrimoine}
       ...
       cohérent avec l'anti-look-ahead de I.2e.}\\[1mm]
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.52\textwidth}\footnotesize
      ...
      \diff{\scriptsize Quiver \textbf{voit} le papier de la Chambre (d'où le gating de
       I.2b) mais est \textbf{aveugle} au papier du Sénat (d'où I.2d).}\\[2mm]
...
\begin{frame}{A2 --- Le portail du Sénat : ce que rend la recherche}
  ...
  {\footnotesize la liste réelle des dépôts (I.1c) --- \textbf{un lien par dépôt} :
...
\begin{frame}{A7 --- Détails : la couverture des entrées/sorties, les pièges d'identité}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.48\textwidth}\footnotesize
      \textbf{pièges d'identité réels, réparés} (I.3) :\\[1mm]
      {\scriptsize
