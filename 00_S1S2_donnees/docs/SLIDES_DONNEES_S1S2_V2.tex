...
% avant le Sénat EN ENTIER (1d → 1e → 1f) — l'alternance précédente « coupait la
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tabular}{@{}p{0.55cm}>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}p{3.4cm}@{}}
        \etape{0}{Le pipeline}{le flux complet, en une image}
        \etape{1}{Acquisition \& Tri}{la Chambre en entier, puis le Sénat en entier}
        \etape{2}{Extraction}{4 voies · les 2 dates · le corpus}
        \etape{3}{Identité}{le nom → l'élu officiel}
        \etape{4}{Tickers}{le symbole boursier}
        \etape{5}{Secteurs}{GICS → ETF}
        \etape{6}{Dédup}{l'empreinte 7 champs}
        \etape{7}{Entonnoir}{ce qu'on retire, et pourquoi}
        \etape{8}{Table finale}{à quoi ressemble la valeur}
      \end{tabular}}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{1a · Acquisition Chambre --- récupérer les dépôts depuis l'index annuel}
  \begin{columns}[T]
    \begin{column}{0.40\textwidth}\centering
      \fcolorbox{black!25}{white}{\includegraphics[width=0.95\linewidth,height=22mm,keepaspectratio]{slides/site_house_clerk.png}}\\[0.8mm]
    ...
    \begin{column}{0.57\textwidth}\centering
      \includegraphics[width=0.94\linewidth,height=37mm,keepaspectratio]{slides/fichier_index_xml.png}\\[0.5mm]
...
\begin{frame}{1c · Tri Chambre --- distinguer les PTR lisibles des scannés}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}\centering
      \fcolorbox{helec}{white}{\includegraphics[width=0.58\linewidth,height=0.40\textheight,keepaspectratio]{sources/house_digital.png}}\\[1.2mm]
      \badge{helec}{PDF lisible --- 71\,\%}\\[0.6mm]
      {\scriptsize couche texte, lu directement \;·\; DocID en \co{2\ldots}}
    \end{column}
    \begin{column}{0.46\textwidth}\centering
      \fcolorbox{hocr}{white}{\includegraphics[width=0.58\linewidth,height=0.40\textheight,keepaspectratio]{sources/house_ocr.png}}\\[1.2mm]
      \badgel{hocr}{PDF scanné --- 29\,\%}\\[0.6mm]
      {\scriptsize image, à lire par \textbf{OCR} \;·\; DocID en \co{8\ldots}/\co{9\ldots}}
    \end{column}
  \end{columns}
  \vspace{2.5mm}
  {\footnotesize\color{black!60} deux mondes, deux extracteurs : le texte se \textbf{parse}
   (2a), l'image passe par l'\textbf{OCR} (2b)}
\end{frame}

% ── 1d · Acquisition Sénat ──
\begin{frame}{1d · Acquisition Sénat --- accéder au portail eFD}
  \begin{columns}[c]
    \begin{column}{0.60\textwidth}\centering
      \fcolorbox{black!25}{white}{\includegraphics[width=0.97\linewidth,height=0.50\textheight,keepaspectratio]{slides/site_senate_efd.png}}\\[0.5mm]
      {\scriptsize\color{black!60} \emph{efdsearch.senate.gov} --- la page d'accès}
    \end{column}
    \begin{column}{0.36\textwidth}
      \badge{alertred}{la barrière : accord + jeton (CSRF)}\\[3mm]
      {\footnotesize
      · une \textbf{case obligatoire} : accepter l'accord avant toute recherche\\[2mm]
      · notre collecteur l'accepte, récupère le \textbf{jeton de session}\\[2mm]
      · ensuite seulement : on peut \textbf{interroger} le portail}
    \end{column}
  \end{columns}
  \vspace{1.5mm}
...
% ── 1e · Tri Sénat · type (Senator × PTR + liste + 2 graphes : type & déposant) ──
\begin{frame}{1e · Tri Sénat --- choisir les PTR : Senator × PTR, puis la liste}
  \begin{columns}[T]
    \begin{column}{0.40\textwidth}\centering
      \fcolorbox{black!25}{white}{\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{slides/site_senate_form.png}}\\[0.6mm]
      {\scriptsize\color{black!60} on coche \textbf{Senator} × \textbf{PTR} → la liste des dépôts}
    \end{column}
    \begin{column}{0.57\textwidth}
      \badge{selec}{2\,160 \emph{Periodic Transactions} = \textbf{42\,\%} des \textbf{5\,096} dépôts}\\[0.8mm]
      {\scriptsize\color{black!55} corpus \textbf{2014-2026} --- tous types}\\[2mm]
      {\scriptsize \textbf{par \emph{Report Type}} \;{\color{black!45}(somme = 100\,\%)}}\\[1mm]
      \begin{tikzpicture}
        \node[font=\tiny, anchor=east] at (0,0.6) {\emph{Annual}};
        \fill[dataC] (0.1,0.47) rectangle (1.82,0.73);
        \node[font=\tiny, anchor=west] at (1.92,0.6) {2 165 · 42\,\%};
        \node[font=\tiny, anchor=east] at (0,0.0) {\emph{Periodic Transactions}};
        \fill[selec] (0.1,-0.13) rectangle (1.80,0.13);
        \node[font=\tiny, anchor=west] at (1.90,0.0) {\textbf{2 160 · 42\,\%}};
        \node[font=\tiny, anchor=east] at (0,-0.6) {\emph{Due Date Extension}};
        \fill[dataC] (0.1,-0.73) rectangle (0.70,-0.47);
        \node[font=\tiny, anchor=west] at (0.80,-0.6) {753 · 15\,\%};
        \node[font=\tiny, anchor=east] at (0,-1.2) {\emph{Blind Trusts} · \emph{Other}};
        \node[font=\tiny, anchor=west] at (0.12,-1.2) {$<1\,\%$};
      \end{tikzpicture}\\[2mm]
      {\scriptsize \textbf{par \emph{Filer Type}} \;{\color{black!45}(tous déposants · somme = 5\,096)}}\\[1mm]
      \begin{tikzpicture}
        \fill[selec] (0,0) rectangle (3.30,0.30);
        \fill[selec!55] (3.30,0) rectangle (4.50,0.30);
        \fill[socr] (4.50,0) rectangle (5.00,0.30);
        \node[font=\tiny, color=white] at (1.65,0.15) {Senator 66\,\%};
        \node[font=\tiny] at (3.90,0.15) {Former 24};
        \node[font=\tiny, anchor=west] at (5.06,0.15) {Candidate 10};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  \vspace{2mm}
  {\footnotesize le PTR est \textbf{un type parmi d'autres} --- on \textbf{choisit les PTR},
   les autres \textbf{dans une autre partie} \;·\; {\color{black!55}les candidats ne tradent
   quasi pas (13 PTR) ; \textbf{31\,\%} des PTR = anciens sénateurs}}
\end{frame}

% ── 1f · Tri Sénat · format (page web vs papier scanné) ──
\begin{frame}{1f · Tri Sénat --- distinguer page web et papier scanné}
  \centering
  \vspace{1mm}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}\centering
      \fcolorbox{selec}{white}{\includegraphics[width=0.72\linewidth,height=0.42\textheight,keepaspectratio]{sources/senate_digital.png}}\\[1.5mm]
      \badge{selec}{page web --- 1\,780}\\[0.8mm]
      {\scriptsize table déjà propre, lue directement}
    \end{column}
    \begin{column}{0.46\textwidth}\centering
      \fcolorbox{socr}{white}{\includegraphics[width=0.52\linewidth,height=0.42\textheight,keepaspectratio]{slides/senat_papier_droit.png}}\\[1.5mm]
...
% ── 1g · Le manuscrit ──
\begin{frame}{1g · Le manuscrit --- le vrai point dur de l'image scannée}
  \centering
  ...
  \diff{le manuscrit existe des \textbf{deux} côtés --- mais il sera \textbf{écarté} à la
   ...
   \textbf{vérifiabilité} (2b et 2d).}
\end{frame}

% ── 1h · Bilan du tri : 4 sous-corpus → 4 extracteurs (charnière avant l'Extraction) ──
\begin{frame}{1h · Bilan du tri --- quatre sous-corpus, quatre extracteurs}
  \centering
  \vspace{2mm}
  {\footnotesize la même donnée réglementaire arrive sous \textbf{4 formats}
   (chambre × format) --- deux mondes \textbf{texte}, deux mondes \textbf{image} :}\\[3.5mm]
  \begin{columns}[T]
    \begin{column}{0.24\textwidth}\centering
      ...
      {\scriptsize \textbf{texte}\\→ \textbf{parsing} (2a)}
    \end{column}
    \begin{column}{0.24\textwidth}\centering
    ...
    \begin{column}{0.24\textwidth}\centering
      ...
      {\scriptsize \textbf{image}\\→ \textbf{OCR} (2d)}
    \end{column}
  \end{columns}
  \vspace{4mm}
  \idem{\textbf{chaque chambre a son monde texte et son monde image} --- au final
   \textbf{57,5\,\%} du corpus vient d'\textbf{images} (le gros des transactions), et
   chaque voie reçoit son \textbf{extracteur dédié} : c'est le plan des \textbf{4 slides
   d'Extraction} qui suivent.}
\end{frame}
...
% ── 2a · Extraction Chambre, texte ──
\begin{frame}{2a · Extraction Chambre · texte --- un PDF n'est pas un tableau}
  \begin{columns}[T]
...
% ── 2b · Extraction Chambre, image ──
\begin{frame}{2b · Extraction Chambre · image --- Vision lit tout, le manuscrit est écarté}
  \centering
...
% ── 2c · Extraction Sénat, texte ──
\begin{frame}{2c · Extraction Sénat · texte --- une table déjà propre, stable sur 13 ans}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}\centering
    ...
    \begin{column}{0.42\textwidth}
      ...
      \colorbox{warnorange!12}{\parbox{0.96\linewidth}{\scriptsize\vspace{0.4mm}
        ...
        cette seule voie porte \textbf{93\,\%} du dédoublonnage (6)\vspace{0.4mm}}}
    \end{column}
  \end{columns}
  \vspace{1mm}
  \diff{là où la \chambre{} impose de \textbf{reconstruire} un PDF (2a), le \senat{}
...
% ── 2d · Extraction Sénat, image ──
\begin{frame}{2d · Extraction Sénat · image --- le papier est gardé, faute de mètre externe}
  \begin{columns}[c]
    ...
    \begin{column}{0.66\textwidth}
      ...
      {\footnotesize \textbf{même moteur} qu'à la Chambre (2b) --- mais
...
% ── 2e · Extraction : les deux dates ──
\begin{frame}{2e · Ce qu'on extrait de chaque ligne : deux dates, une seule est fiable}
  \centering
  \begin{columns}[T]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      ...
      {\footnotesize le papier n'est pas un détail : c'est \textbf{la majorité du
       corpus} --- d'où l'effort d'OCR de 2b et 2d}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{II.1 · L'univers --- 100\,\% des 8\,252 PTR officiels sont traités}
  ...
  \begin{tikzpicture}[node distance=6mm]
    ...
    \node[step=alertred, text width=3.2cm, minimum height=1.3cm, font=\footnotesize, right=6mm of c] (d)
      {\textbf{\large 582}\\{\scriptsize écartés --- manuscrits, règle écrite (2b)}};
    \draw[arr] (a) -- (b);
  \end{tikzpicture}}\\[3mm]
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \choixbox{les transactions hors PTR alimentent le \textbf{patrimoine}
       ...
       cohérent avec l'anti-look-ahead de 2e.}\\[1mm]
