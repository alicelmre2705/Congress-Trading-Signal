...
*Résumé explicatif : vision d'ensemble et résultats clés, sans le détail des fonctions. Le détail technique complet est dans [RAPPORT_QUALITE.md](RAPPORT_QUALITE.md) ; la traçabilité des vérifications est dans [AUDIT_DONNEES_2014_2026.md](AUDIT_DONNEES_2014_2026.md).*
...
- **Référentiel public des élus** (identité, parti, commissions) + **référentiels transverses** (`data/reference/` : renommages/délistages de tickers, carte secteurs, snapshots de commissions par Congrès).
...
- Nom libre → **identifiant officiel unique** (bioguide) via référentiel + matcher tolérant (surnoms, homonymes — chaque déposant est rattaché au bon élu, cas d'homonymie compris).
- **Résultat : 100 % des lignes rattachées, les deux chambres.**
...
- **Résultat : 58 728 transactions** — le parseur gère la police pré-2018 (petites capitales) et les gabarits mixtes (union de deux lecteurs) : les achats et les ventes sont extraits à parité.
...
- **Manuscrit (cluster C) exclu par défaut** (dates trop incertaines) — politique **rejouable** : 582 documents écartés par cette règle (liste versionnée), avec exceptions explicites : 3 gros déposants réintégrés (les écarter ferait perdre trop de transactions corroborées) + 33 documents relus et validés à la main.
...
- Cascade factuelle (yfinance) → LLM → corrections manuelles, source tracée. Les **ETF diversifiés n'ont pas de secteur GICS** (classe `etf_broad` dans la carte transverse — un secteur n'aurait pas de sens) et les symboles recyclés sont datés.
...
- **Complétude prouvée** : 100 % de l'index officiel House traité ; Sénat = sur-ensemble strict de senate-stock-watcher (6 231 transactions externes 2014-2020 : **100 % retrouvées** — seule divergence : la façon de représenter les opérations d'échange, pas des transactions manquantes).
- **Identité : 100 %** ; montants renseignés 99,4 % (milieu exact de fourchette, convention unique).
...
- **Sources indépendantes** : senate-stock-watcher (100 % couvert) et mirror house-stock-watcher (99,5 % post-2018).

## 8. Qualité & reproductibilité
- **Rapport qualité automatique** régénérable offline (`python -m common.quality`), déterministe, avec section « couverture vs univers officiel ».
- **Jeux de référence figés (« golden »)** : 230 fichiers Chambre + 138 Sénat, comparés octet à octet à chaque exécution (zéro écart toléré) ; **10/10 tests** de régression verts ; chaque transformation reproduite depuis les colonnes figées.
...
## 10. Conclusion & suite
- Une couche de données **complète (prouvée contre l'univers officiel), identifiée, enrichie, validée par trois sources externes et reproductible** — prête pour la recherche.
