...
\captionof{figure}{Du symbole déclaré au prix exploitable (côté recherche, notebooks 05/06 \S3).}
\end{minipage}
\end{center}

\begin{itemize}
  \item \textbf{Amont (nettoyage)} : ticker normalisé au format Yahoo ; \textbf{+2\,540} tickers
        récupérés depuis la description de l'actif (double preuve) ; renommages appliqués via le
        référentiel de 84 entrées (FB$\to$META, PCLN$\to$BKNG\ldots{} — 2\,817 lignes re-symbolisées) ;
        3\,542 lignes sur délistés \emph{typés} (faillite $\ne$ rachat).
  \item \textbf{Sans prix (1\,268)} : symboles que Yahoo \emph{ne sert plus} — délistés sans
        successeur coté.
  \item \textbf{Garde-fou anti-corruption} : série exclue si prix min $< 0{,}10$~\$ ou saut journalier
        $> 300\,\%$ (61 séries, ex.\ \code{DAIUF} : prix négatifs).
  \item \textbf{Résultat} : \textbf{3\,289 tickers} exploitables ; couverture \textbf{87{,}8\,\%} des
        trades (74\,\% en 2014 $\to$ 99\,\% en 2026) $\Rightarrow$ léger biais de survie, à garder en
        tête côté rendements.
\end{itemize}
